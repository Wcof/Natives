...
// 导出坐标映射与基础计算工具，供 chart-drawings 等交互层消费
actions.priceToY = priceToY;
actions.yToPrice = yToPrice;
actions.timeIndexToX = timeIndexToX;
actions.xToTimeIndex = xToTimeIndex;
actions.applyMagnet = applyMagnet;
actions.getChartBounds = function() { return bounds; };
...
function redrawChart() {
  ...
  if (actions.redrawInteractive) {
    actions.redrawInteractive();
  }
  renderVolumeChart();
}
...
pBtns.forEach(function(b) {
  b.addEventListener("click", function() {
    ...
    loadChartData();
    if (actions.loadDrawings) actions.loadDrawings();
  });
});
