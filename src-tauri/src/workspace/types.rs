...
/// stays a lossless JS `number` (below 2^53).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
///
/// `revision` (A-033) mirrors `WorkspaceSnapshot::revision` semantics: the
/// session carries the open workspace's full collections, so the same content
/// fingerprint applies.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
