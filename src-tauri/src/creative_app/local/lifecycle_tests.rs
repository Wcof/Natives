...
fn stale_run_exit_does_not_stop_newer_run() {
    ...
    let i1 =
        crate::creative_app::runtime_store::create_instance(&conn, &app, None, "local_process")
            .unwrap();
    crate::creative_app::runtime_store::mark_running(&conn, &i1, &[], None, None, None).unwrap();
...
async fn local_process_driver_e2e_writes_services_and_releases_resources() {
    ...
    .unwrap();
    let instance_id =
        crate::creative_app::runtime_store::create_instance(&conn, &app_id, None, "local_process")
            .unwrap();
    ...
    let services = crate::creative_app::service_store::list_services(&conn, &instance_id).unwrap();
