...
impl WebviewGateway for FakeGateway {
    ...
    fn show(&self, label: &str, _app_id: &str, _url: &str, _bounds: BrowserBounds) -> Result<()> {
...
fn open_requires_running_instance() {
    ...
    .unwrap();
    let surface_id = surface_store::create_surface(&conn, "app-1", "main", "Main", None).unwrap();
    let gw = FakeGateway::default();
    let mut c = conn;
...
fn open_two_surfaces_get_distinct_labels() {
    ...
    let surface2 = surface_store::create_surface(&c, "app-1", "embed", "Embed 2", None).unwrap();
    let w2 = WindowController::open(
        &gw,
        &mut c,
        "app-1",
...
fn open_refuses_beyond_live_window_cap() {
    ...
    let extra = surface_store::create_surface(&c, "app-1", "embed-extra", "Extra", None).unwrap();
    let err = WindowController::open(
        &gw,
        &mut c,
        "app-1",
