...
fn decode_lsof_path(path: &str) -> String {
    crate::terminal_process::decode_lsof_path(path)
}

/// 简单的 URL 解码（OSC 7 路径可能包含 %20 等编码）
fn url_decode_path(s: &str) -> String {
    crate::terminal_process::url_decode_path(s)
}
...
// (moved to `terminal_process.rs`; re-exported for API compatibility)
// ──────────────────────────────────────────────
pub use crate::terminal_process::{detect_binary, launch_binary};

// ──────────────────────────────────────────────
// GhosttyManager — dedicated Ghostty lifecycle
// (moved to `terminal_ghostty.rs`; re-exported for API compatibility)
// ──────────────────────────────────────────────
pub use crate::terminal_ghostty::GhosttyManager;
