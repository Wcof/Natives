...
fn local_project_static_url_is_revoked_after_stop() {
    ...
    .unwrap();
    let iid = crate::creative_app::runtime_store::create_instance(&f.conn, &app, None, "host_http")
        .unwrap();
    crate::creative_app::runtime_store::mark_running(&f.conn, &iid, &[], None, None, None).unwrap();
