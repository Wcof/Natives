// tokenusage 模块计量与指标层（ADR-0028 D3）。
//
// 统一将 model_usage_overview / model_usage_analysis / model_usage_events
// 转换为归一化的 AI 计量数据集。所有数据来自 Model Host 预聚合（R-P 纪律）。
...
export function toPoints(trend, key) {
  return (trend || [])
...
export function toRank(items, valueKey) {
...
// userTimezone 返回浏览器当前 IANA 时区（未显式传入时的默认值）；
