...
export function createPreviewControllers({ $, call, t, session, setStatus, toast, formatSize, parentAndName, pathParts, entryIcon, iconElement, iconAction, isTextItem, TEXT_KINDS, isHostConnected, selectedItems, renderSelection, loadDirectory, remember, revealPath, link }) {
  ...
  function appendEditorPreviewActions(item) {
    const toolbar = $('preview-body').querySelector('.editor-toolbar');
    if (!toolbar || toolbar.querySelector('[data-editor-preview-actions]')) return;
    const actions = document.createElement('span'); actions.className = 'preview-actions'; actions.dataset.editorPreviewActions = 'true';
    ...
    appendCopyPathAction(actions, item); toolbar.append(actions);
  }
  ...
  async function renderPreviewSelection() {
  ...
  function renderTextEditor(item, result) {
    const readOnly = Boolean(result.truncated); const generation = session.previewGeneration; editorState = { path: item.path, mtime: Number(result.mtime) || Number(item.mtime) || 0, savedContent: result.content || '', dirty: false, saveTimer: undefined, conflict: false, readOnly }; const body = $('preview-body'); const meta = document.createElement('p'); meta.className = 'preview-meta muted'; meta.textContent = `${formatSize(result.size)} · ${result.encoding || 'utf-8'}${readOnly ? ` · ${t('previewTruncated', '内容已截断')}` : ''}`; const toolbar = document.createElement('div'); toolbar.className = 'editor-toolbar'; const stateLabel = document.createElement('span'); stateLabel.className = 'muted'; stateLabel.textContent = readOnly ? t('largeTextReadOnly', '大文件仅显示预览，编辑已禁用') : t('saved', '已保存'); const save = document.createElement('button'); save.textContent = t('save', '保存'); save.disabled = true; save.hidden = readOnly; save.onclick = () => queueEditorSave(true); const toggle = document.createElement('button'); toggle.className = 'markdown-mode-toggle'; toggle.textContent = t('readMode', '阅读'); toggle.hidden = !/\.(md|markdown|mdx)$/i.test(item.name); toggle.onclick = () => { const reading = toggle.dataset.mode !== 'reading'; toggle.dataset.mode = reading ? 'reading' : 'source'; toggle.textContent = t(reading ? 'sourceMode' : 'readMode', reading ? '源码' : '阅读'); editor.hidden = reading; body.querySelectorAll('.markdown-preview,.markdown-images').forEach((node) => { node.hidden = !reading; }); }; toolbar.append(stateLabel, save, toggle); const editor = document.createElement('textarea'); editor.className = 'file-editor'; editor.value = result.content || ''; editor.readOnly = readOnly; editor.spellcheck = false; editor.setAttribute('aria-label', t('editorLabel', '文件编辑器')); editor.addEventListener('input', () => { if (!editorState || editorState.readOnly) return; editorState.dirty = editor.value !== editorState.savedContent; editorState.conflict = false; stateLabel.textContent = editorState.dirty ? t('unsaved', '未保存') : t('saved', '已保存'); save.disabled = !editorState.dirty; scheduleEditorSave(); }); editor.addEventListener('keydown', (event) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 's') { event.preventDefault(); if (!editorState?.readOnly) queueEditorSave(true); } }); body.append(meta, toolbar, editor); renderJsonPreview(item, result.content || '', body, generation); renderCsvPreview(item, result.content || '', body, generation); renderMarkdownSafePreview(item, result.content || '', body, generation); renderMarkdownImages(item, result.content || '', body, generation); body.querySelectorAll('.markdown-preview,.markdown-images').forEach((node) => { node.hidden = true; }); editor.focus(); const view = editorViewStates.get(item.path); if (view) { editor.scrollTop = Math.max(0, Number(view.scrollTop) || 0); editor.setSelectionRange(Math.min(Number(view.selectionStart) || 0, editor.value.length), Math.min(Number(view.selectionEnd) || 0, editor.value.length)); }
  }
