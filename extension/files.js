...
function showContextMenu(x, y, item) { const menu = $('context-menu'); menu.replaceChildren(); const isText = item && !item.isDir && (item.kind === 'text' || isTextFile(item)); const actions = item ? [['open', 'open', () => openItem(item)], ['previewAction', 'preview', () => { const show = () => { session.selectedPaths = new Set([item.path]); session.lastSelectedIndex = session.entries.findIndex((entry) => entry.path === item.path); renderSelection(); }; if (editorState()?.dirty) guardDirty(show); else show(); }], ...(isText ? [['editAction', 'edit', () => { const showEdit = () => { session.selectedPaths = new Set([item.path]); session.lastSelectedIndex = session.entries.findIndex((entry) => entry.path === item.path); renderSelection({ editMode: true }); }; if (editorState()?.dirty) guardDirty(showEdit); else showEdit(); }]] : []), ...(item.isDir ? [['diskUsage', 'diskUsage', () => showDiskUsage(item.path)]] : []), ...(item.kind === 'archive' && /\.(zip|jar|tar|tgz|tbz2?|txz|tar\.(gz|bz2|xz|zst))$/i.test(item.name) ? [['extractArchive', 'extractArchive', () => extractArchive(item)]] : []), ['reveal', 'reveal', () => revealSelected()], ['copyPath', 'copyPath', () => copyPathSelected()], ['rename', 'rename', () => renameSelected()], ['copy', 'copy', () => transfer('copy')], ['duplicate', 'duplicate', () => duplicateSelected()], ...(item.isDir && ops.getClipboard() ? [['paste', 'paste', () => pasteClipboard(item.path)]] : []), ['move', 'move', () => transfer('move')], ['trash', 'trash', () => trashSelected()]] : [['newFolder', 'newFolder', () => createEntry('directory')], ['newFile', 'newFile', () => createEntry('file')], ...(ops.getClipboard() ? [['paste', 'paste', () => pasteClipboard()]] : [])];
