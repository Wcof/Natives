/**
 * File operations atomic capabilities (增删改查):
 * 增 createEntry / importFileList / importDroppedImageUrls
 * 删 trashSelected / retryTrashOperation
 * 改 renameSelected / pasteClipboardNow / setClipboard / moveDroppedPaths / copyImageIntoEditor
 * 查 openItem / revealPath / openEditorSelected / copyPathSelected(+For) / copyFileSelected
 * plus duplicate / transfer / extractArchive / createZip / batch runner / retry bookkeeping.
 * Page dependencies are injected via createFileOperations(); `link` is wired by
 * the composition root after the preview capability exists (lazy cycle break).
 */
export function createFileOperations({ $, call, t, session, setStatus, toast, updateProgress, setOperationCancelable, openModal, loadDirectory, renderSelection, renderStatusBar, selectedItems, navigate, parentAndName, kindFromName, markSelfOpened, link }) {
  ...
  // 查：open / reveal / editor / copy path
  ...
  // 改/移动：rename / transfer / duplicate / drop moves / uri copies
