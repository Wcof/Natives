...
export function renderNotificationCenter(
  ...
  header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:10px 16px;border-bottom:1px solid var(--border);';
  const titleSpan = document.createElement('span');
  titleSpan.style.cssText = 'font-size:13px;font-weight:600;color:var(--text-secondary)';
  titleSpan.textContent = 'Notifications';
  header.appendChild(titleSpan);
  const markAllBtn = document.createElement('button');
  markAllBtn.className = 'btn-ghost';
  markAllBtn.textContent = 'Mark all read';
  markAllBtn.style.cssText = 'font-size:11px;color:var(--primary);background:none;border:none;cursor:pointer;';
  ...
  if (notifications.length === 0) {
    ...
    empty.style.cssText = 'padding:40px 20px;text-align:center;color:var(--text-secondary);font-size:13px;';
  ...
  for (const notif of notifications) {
    ...
    if (notif.body) {
      ...
      bodyEl.style.cssText = 'font-size:12px;color:var(--text-secondary);margin:2px 0 0';
      bodyEl.textContent = notif.body;
      left.appendChild(bodyEl);
    }

    const timeEl = document.createElement('span');
    timeEl.style.cssText = 'font-size:10px;color:var(--text-secondary);margin-top:4px;display:block';
    ...
    if (!notif.read) {
      ...
      markBtn.style.cssText = 'font-size:10px;color:var(--primary);background:none;border:none;cursor:pointer;';
...
export function renderErrorPage(
  ...
  title.style.cssText = 'font-size:18px;font-weight:600;color:var(--text);margin-bottom:8px;';
  title.textContent = error.userMessage;
  container.appendChild(title);

  if (error.actionHint) {
    const hint = document.createElement('p');
    hint.style.cssText = 'font-size:13px;color:var(--text-secondary);margin-bottom:24px;';
