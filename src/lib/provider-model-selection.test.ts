...
describe('selectAssistantModel',()=>{
  it('returns null when empty',()=>assert.equal(selectAssistantModel([]),null));
  it('returns first ready',()=>{const r=selectAssistantModel([{id:'p1',provider_type:'o',display_name:'O',has_active_key:true,default_model:'gpt-4o',models:[{id:'gpt-4o'}]}]);assert.notEqual(r,null);assert.equal(r!.providerId,'p1');assert.equal(r!.modelId,'gpt-4o');});
});
describe('connectionFingerprint',()=>{
  it('same',()=>assert.equal(connectionFingerprint('https://a.com','sk-abc'),connectionFingerprint('https://a.com','sk-abc')));
  it('different',()=>assert.notEqual(connectionFingerprint('https://a.com','sk-abc'),connectionFingerprint('https://a.com','sk-xyz')));
});
describe('normalizeDiscoveredModels',()=>{it('filters empty id',()=>{const r=normalizeDiscoveredModels([{id:'gpt-4'},{id:''},{id:'  '}]);assert.equal(r.length,1);assert.equal(r[0]!.id,'gpt-4');});});
