...
exports.findSubsequence = findSubsequence;
exports.findStreaks = findStreaks;
exports.calculateScore = calculateScore;
exports.indexFileForSearch = indexFileForSearch;
exports.searchFtsIndex = searchFtsIndex;
exports.grepContent = grepContent;
 ...
 */
function findSubsequence(query, target) {
 ...
 * @returns 连续段长度数组
 */
function findStreaks(positions) {
    const streaks = [];
    let currentStreak = 1;
 ...
 * 计算词边界匹配数
 */
function countWordBoundaryMatches(positions, filename) {
    let count = 0;
 ...
 * @returns 分数，不匹配返回 -1
 */
function calculateScore(query, filename, isDir, mtime) {
    let score = 0;
    // 1. 子序列匹配基础分
...
function hasRipgrep() {
    if (_hasRipgrep !== null)
        return _hasRipgrep;
    try {
        (0, child_process_1.execFileSync)('rg', ['--version'], { stdio: 'ignore', timeout: 3000 });
        _hasRipgrep = true;
    }
    catch {
        _hasRipgrep = false;
 ...
 */
function indexFileForSearch(filePath, content) {
    const db = getFtsDb();
    if (!db)
        return;
 ...
 */
function searchFtsIndex(query, limit = 50) {
    const db = getFtsDb();
    if (!db)
        return [];
    try {
        const rows = db.prepare(`SELECT path, snippet(file_contents, 1, '<mark>', '</mark>', '...', 40) AS preview
       FROM file_contents WHERE file_contents MATCH ? ORDER BY rank LIMIT ?`).all(query, limit);
        return rows.map((r) => ({
            path: r.path,
...
async function grepWithRipgrep(query, root, options) {
    ...
    const output = (0, child_process_1.execFileSync)('rg', args, { encoding: 'utf-8', timeout: 30000, maxBuffer: 10 * 1024 * 1024 });
    const results = [];
 ...
 * @returns 匹配结果
 */
async function grepContent(query, root, options) {
    const maxResults = options?.maxResults || 50;
    ...
    async function searchFile(filePath) {
        ...
        try {
            ...
            for (let i = 0; i < lines.length && results.length < maxResults; i++) {
                ...
                if (matchIdx !== -1) {
                    const preview = lines[i].substring(Math.max(0, matchIdx - 20), Math.min(lines[i].length, matchIdx + query.length + 20)).trim();
                    results.push({
                        path: filePath,
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    if (entry.name.startsWith('.'))
                        continue;
                    // 跳过常见构建/依赖目录
                    if (['node_modules', 'dist', '.next', 'build', 'out', '__pycache__'].includes(entry.name))
                        continue;
 ...
 * @returns 排序后的搜索结果
 */
async function searchFiles(query, root, options) {
 ...
 * @returns 匹配结果
 */
async function spotlightSearch(query, root) {
    // 优先使用 mdfind (macOS)
    if (process.platform === 'darwin') {
        try {
            const output = await (0, exec_file_1.execFilePromise)('mdfind', ['-onlyin', root, '-interpret', query]);
