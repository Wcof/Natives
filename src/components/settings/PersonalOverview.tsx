...
export default function PersonalOverview({ locale, onNavigate }: PersonalOverviewProps) {
  ...
  const [metadata, setMetadata] = useState<UsageCacheMetadata | null>(null);
  ...
  const loadOverview = useCallback(async () => {
    ...
    const results = await Promise.allSettled([
      (async () => {
        const api = window.nativesAPI?.usage;
        if (!api?.getCached) throw new Error('usage API not available');
        return api.getCached({ preset: range, timeZone, projectPath: null });
      })(),
      (async () => {
    ...
    const [usageResult, projectResult, diskResult] = results;

    if (usageResult.status === 'fulfilled') {
      const result = usageResult.value as UsageCacheReadResult;
      ...
      nextErrors.push({ scope: 'usage', message: classifyError(usageResult.reason).userMessage });
    }
  ...
  const syncUsage = useCallback(async () => {
    ...
    try {
      const api = window.nativesAPI?.usage;
      if (!api?.sync) throw new Error('usage API not available');
      const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC';
      const result = await api.sync({
        timeZone,
        currentView: { preset: range, timeZone, projectPath: null },
...
function MetricCard({
  ...
  return (
    <section className={`${styles.card} ${styles.metricCard}`}>
      ...
      <div className={styles.metricValue} title={value}>{value}</div>
...
function ActivityStat({
  ...
  return (
    <div className={styles.activityStat}>
      ...
      <div>
        <div className={styles.activityStatValue}>{value === null ? t(locale, 'settings.overviewUnavailable') : fmtCount(value, locale)}</div>
