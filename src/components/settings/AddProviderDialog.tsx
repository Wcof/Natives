...
function pn(p: ProviderPreset, l: string) { return (l.startsWith('zh') && p.nameZh) ? p.nameZh : p.name; }
function pd(p: ProviderPreset, l: string) { return (l.startsWith('zh') && p.descriptionZh) ? p.descriptionZh : (p.description || ''); }

export default function AddProviderDialog({ locale, onClose, onSave }: Props) {
  const [search, setSearch] = useState('');
  const [sel, setSel] = useState<ProviderPreset | null>(null);
  ...
  const fil = useMemo(() => {
    ...
    return PROVIDER_PRESETS.filter(p => p.name.toLowerCase().includes(q) || (p.nameZh?.toLowerCase() ?? '').includes(q) || (p.description?.toLowerCase() ?? '').includes(q) || (p.descriptionZh?.toLowerCase() ?? '').includes(q) || p.websiteUrl.toLowerCase().includes(q));
  }, [search]);

  const hs = (preset: ProviderPreset) => { setSel(preset); setName(pn(preset, locale)); setWu(preset.websiteUrl); setBu(preset.baseUrl); setDm(''); setKl('API Key 1'); setKv(''); setMos([]); setFme(null); setDfp(null); setTrr(null); setSve(null); };
  ...
  const hd = async () => {
    if (!sel || !hk || fm) return;
    setFm(true); setFme(null);
    try {
      const a = window.nativesAPI?.provider;
      if (!a?.discoverModels) { setFme(t('settings.modelDiscoveryUnavailable')); return; }
      const ms = normalizeDiscoveredModels(await a.discoverModels({ providerType: sel.providerType || 'openai', baseUrl: bu.trim() || sel.baseUrl, apiKey: kv.trim() }));
          ...
          {sel ? <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING.sm }}>
            ...
            <Field label="Default Model" icon={<Link size={12} />}>
              <div style={{ display: 'flex', gap: SPACING.xs }}>
                ...
                <button type="button" onClick={hd} disabled={!hk || fm} title="Fetch" style={{ ...inp, width: 'auto', display: 'inline-flex', alignItems: 'center', gap: 4, cursor: (!hk || fm) ? 'not-allowed' : 'pointer' }}>
                  {fm ? <Loader size={13} className="animate-spin" /> : <RefreshCw size={13} />} Fetch
                </button>
              </div>
            ...
            <div style={{ marginTop: SPACING.xs, display: 'flex', flexDirection: 'column', gap: SPACING.sm }}>
              <button onClick={htt} disabled={te || !hk || !cum} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, justifyContent: 'center', padding: '6px 12px', borderRadius: BORDER_RADIUS.md, background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', cursor: (te || !cum) ? 'not-allowed' : 'pointer', fontSize: FONT_SIZE.xs, opacity: te ? 0.6 : 1 }}>
                {te ? <Loader size={12} className="animate-spin" /> : <Wifi size={12} />}
                {te ? 'Testing…' : 'Test Connection'}
              </button>
