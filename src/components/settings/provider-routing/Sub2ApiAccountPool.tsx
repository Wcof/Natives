...
export function Sub2ApiAccountPool({ locale, providers, api, onProviderCreated }: { locale: Locale; providers: ProviderSummary[]; api: ProviderRoutingApi | undefined; onProviderCreated: () => Promise<void> }) {
  ...
  return <section className="settings-section-card" style={{ marginTop: SPACING.lg }}>
    ...
    {/* T216 (P1-020): unified create-pool name dialog replaces window.prompt */}
    {showCreatePool && (
      <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '20vh', background: 'var(--overlay)' }} onClick={(e) => { if (e.target === e.currentTarget) { setShowCreatePool(false); setPoolNameDraft(''); } }}>
        <div style={{ width: 360, maxWidth: '90vw', background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: BORDER_RADIUS.md, padding: SPACING.lg, display: 'grid', gap: SPACING.md }}>
