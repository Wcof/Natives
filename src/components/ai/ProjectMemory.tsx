...
export default function ProjectMemory() {
  ...
  const handleRestore = useCallback(async (session: AgentSession) => {
    ...
    try {
      ...
      if (!sessionId) {
        toast('Failed to create terminal session', 'error');
        return;
      }
          ...
          (sessions ?? []).map((s) => (
            <div
              ...
              style={{
                ...
                background: selected === s.id ? 'var(--surface)' : 'var(--surface)',
              ...
              {selected === s.id && (
                <div style={{ marginTop: 8, paddingTop: 6, borderTop: '1px solid var(--border)' }}>
                  {s.filesModified.length > 0 && (
                    <div style={{ fontSize: FONT_SIZE.sm, color: 'var(--text-secondary)', marginBottom: 6 }}>
