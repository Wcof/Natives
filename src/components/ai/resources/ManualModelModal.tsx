...
interface ManualModelModalProps {
  providerId: string;
  connectionId?: string;
  onClose: () => void;
  onSuccess: () => void;
    ...
    <div className="fixed inset-0 bg-[var(--background)]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-lg">{t(locale, 'aiResources.addManual')}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-[var(--muted-foreground)]">Model ID</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="text-xs font-semibold text-[var(--muted-foreground)]">Display Name</label>
            <input
              type="text"
