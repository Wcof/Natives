...
export interface MarkdownTextProps {
  ...
  urlTransform?: MarkdownPreviewProps['urlTransform'];
}
 ...
 */
export default function SafeMarkdown({ source, className, Preview, urlTransform = transformMarkdownUrl }: MarkdownTextProps) {
  const text = source ?? '';
  if (!text.trim()) return null;

  if (Preview) {
    return <MarkdownTextView source={text} className={className} Preview={Preview} urlTransform={urlTransform} />;
  }
