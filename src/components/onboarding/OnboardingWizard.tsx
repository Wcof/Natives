  ...
  return (
    <div style={{
      ...
      <div style={{ width: 480, maxWidth: '90vw' }}>
        {/* Step indicator */}
        <div style={{ display: 'flex', gap: SPACING.xs, marginBottom: SPACING.xxl, justifyContent: 'center' }}>
          {STEPS.map((s) => (
            <div key={s} style={{
        ...
        {step === 'welcome' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: SPACING.sm, justifyContent: 'center' }}>
              <button className="btn" onClick={handleSkip} style={{ fontSize: FONT_SIZE.md }}>{t(locale, 'onboarding.skip')}</button>
              <button className="btn btn-primary" onClick={handleNext} style={{ fontSize: FONT_SIZE.md }}>{t(locale, 'onboarding.getStarted')}</button>
            </div>
          </div>
        )}
        ...
        {step === 'env-check' && (
          <div style={{ textAlign: 'center' }}>
            ...
            {envOk === null ? (
              <button className="btn btn-primary" onClick={handleEnvCheck} disabled={checking} style={{ fontSize: FONT_SIZE.md }}>
              ...
              <div>
                <div style={{ fontSize: FONT_SIZE.xl, color: 'var(--danger)', marginBottom: SPACING.lg }}>{t(locale, 'onboarding.checksFailed')}</div>
                <button className="btn btn-primary" onClick={handleNext} style={{ fontSize: FONT_SIZE.md }}>{t(locale, 'onboarding.continueAnyway')}</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'ai-config' && (
          <div style={{ textAlign: 'center' }}>
            ...
            />
            <div style={{ display: 'flex', gap: SPACING.sm, justifyContent: 'center' }}>
              <button className="btn" onClick={handleNext} style={{ fontSize: FONT_SIZE.md }}>{t(locale, 'onboarding.skip')}</button>
              <button className="btn btn-primary" onClick={() => { handleSaveApiKey(); handleNext(); }} style={{ fontSize: FONT_SIZE.md }} disabled={!apiKey.trim()}>
        ...
        {step === 'module-install' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: SPACING.sm, justifyContent: 'center' }}>
              <button className="btn" onClick={handleNext} style={{ fontSize: FONT_SIZE.md }}>{t(locale, 'onboarding.skip')}</button>
              <button className="btn btn-primary" onClick={() => { window.dispatchEvent(new CustomEvent('navigate', { detail: '__workshop__' })); onComplete(); }} style={{ fontSize: FONT_SIZE.md }}>
