...
interface LocalProjectEditProps {
  app: AppView;
  onSuccess: (updated: AppView) => void;
  onCancel: () => void;
}

export function LocalProjectEdit({ app, onSuccess, onCancel }: LocalProjectEditProps) {
  const locale = useLocale();
  const [title, setTitle] = useState(app.title);
  const [description, setDescription] = useState(app.description || '');
  const [showInSidebar, setShowInSidebar] = useState(app.showInSidebar);
  const [sidebarOrder] = useState<number | undefined>(app.sidebarOrder);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const updated = await appsApi.updateMetadata({
        appId: app.appId,
        title: title.trim(),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        />
        <label htmlFor="local-show-sidebar" className="text-xs text-[var(--text-secondary)] select-none">
