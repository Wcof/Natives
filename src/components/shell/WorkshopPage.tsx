  ...
  useEffect(() => {
    ...
    return api.onLog((ev) => {
      ...
      setLogsText((prev) => {
        ...
        const next = `${prev}\n${line}`;
        // Bound the Renderer log buffer so a long-lived server cannot grow it
        // without limit (batch 7). Drop the oldest once past the cap.
        const CAP = 200_000;
          ...
          {addMenu === 'open' && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1.5 w-52 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-lg p-1 z-20 flex flex-col gap-0.5">
                {/* 旧的硬编码模板「创建」入口已删除（batch 7）：唯一的创作入口是
                   创作台的一句话生成，导入走本地/GitHub 向导。 */}
                <button
                  type="button"
