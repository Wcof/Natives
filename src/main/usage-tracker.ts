...
export function parseUsageWindow(data: any): UsageWindow {
  if (!data || typeof data !== 'object') return { used: 0, limit: 0, resetAt: 0 };
  return {
    used: data.used || 0,
    limit: data.limit || 0,
...
export function calcRemaining(window: UsageWindow): number {
...
export function parseClaudeUsage(data: any): ClaudeUsage {
  return {
