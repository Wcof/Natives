...
/** Daemon-validated auth modes (`mcp.rs` validate_draft whitelist). */
 ...
 * A `null` value is only legal on update: it is the daemon-side sentinel for
 * "keep the stored value for this key" (values are never echoed to the UI).
 */
