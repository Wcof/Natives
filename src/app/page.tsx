...
const WALLPAPERS = [
  'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=800&auto=format&fit=crop', // Pink/blue abstract wave
  'https://images.unsplash.com/photo-1634017839464-5c339ebe3cb4?q=80&w=800&auto=format&fit=crop', // Purple Monterey gradient
...
export default function DashboardPage() {
  ...
  const [elasticity, setElasticity] = useState(0); // 0 by default to disable deforming hover animation
  ...
  const [showWallpaper, setShowWallpaper] = useState(false); // Hidden by default
  const [wallpaperIndex, setWallpaperIndex] = useState(0);
  const [showBlobs, setShowBlobs] = useState(false); // Hidden by default

  // --- Config Database Persistence (US16) ---
  ...
  // Save configuration to database with a 500ms debounce when settings change
  useEffect(() => {
    const api = window.nativesAPI;
  ...
  const toggleLanguage = async () => {
    ...
    try {
      if (window.nativesAPI?.setLocale) {
        await window.nativesAPI.setLocale(nextLocale);
      }
    } catch (err) {
      console.warn('Failed to set native locale:', err);
    }
  ...
  // Window draggability helper styles (R-U13)
  ...
  return (
    ...
    >
      ...
      <LiquidGlass
        ...
        padding="0px" // Manage padding internally to allow full-bleed headers
        mode={mode}
      ...
      >
        <div
          ...
          style={{
            ...
            ...dragStyle, // Drag zone on card body background
          }}
        >
          {/* Header */}
          ...
          {/* Segmented Tab Bar (R-U13: no-drag) */}
          <div
          ...
          <div className="flex-1 flex flex-col justify-between" style={noDragStyle}>
            {activeTab === 'control' ? (
              ...
              <div className="flex flex-col gap-4 animate-fade-in">
                ...
                <div className="flex flex-col gap-3 p-4 bg-white/5 border border-white/5 rounded-2xl">
                  ...
                  <div>
                    ...
                    <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full transition-all duration-[600ms] ease-out-expo"
                  ...
                  <div className="flex justify-between items-center pt-1 mt-1 border-t border-white/5 text-[10px]">
                    ...
                    <span
                      className={`flex items-center gap-1 font-bold px-2 py-0.5 rounded-lg border ${
                        isSandboxActive
                    ...
                    >
                      {isSandboxActive ? (
                        <>
                          <ShieldCheck size={11} />
                          {t(locale, 'controlHub.sandboxActive')}
                        </>
                      ) : (
                        <>
                          <ShieldAlert size={11} />
                          {t(locale, 'controlHub.sandboxInactive')}
                        </>
                      )}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <button
                    onClick={() => {
                      setClicks((c) => c + 1);
                      setMemoryUsage((m) => Math.min(256, m + 6));
                    }}
                  ...
                  <button
                    onClick={() => setIsSandboxActive((p) => !p)}
              ...
              <div className="flex flex-col gap-4 animate-fade-in overflow-y-auto max-h-[385px] pr-1.5 scrollbar-thin">
                ...
                <div className="flex flex-col gap-3 p-4 bg-white/5 border border-white/5 rounded-2xl">
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-3">
                    ...
                    <div className="flex gap-1 bg-black/35 p-0.5 rounded-lg border border-white/5">
                      ...
                      <button
                        onClick={() => setTheme('liquid-glass-light')}
                        className={`px-2 py-1 text-[9px] font-bold rounded-md transition-all ${
                ...
                {/* Firefox / Safari support warning */}
