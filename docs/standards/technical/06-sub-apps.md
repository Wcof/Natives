...
> **2026-09-13 主入口澄清（用户明确决定）**：完整产品必须包含可在系统“应用程序”找到的 `/Applications/Natives.app`；只允许该主产品正常应用登记、图标和薄首次引导/诊断窗口。本文禁止独立 `.app`、Dock/LaunchServices 的条款约束基金等内置模块，不得用来隐藏 Natives 主程序。双击主程序自动打开 Chrome，按实际浏览器能力引导加载随包扩展；细则见 ADR-0029 与唯一实施方案 §1.1。
