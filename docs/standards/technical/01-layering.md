...
> **关联 ADR**: [ADR-0008](../../adr/0008-electron-to-tauri-migration.md)
> **关联源文件**: `src-tauri/`、`src/app/`、`src/components/`

---

## 一、本篇要约束什么

Natives 是 Tauri v2 + Next.js + SQLite 的多进程、多层系统。最大的技术风险是**跨层越权调用**（如前端直连 SQLite、插件直接 require Node 模块）。本篇把进程边界、模块分层、依赖方向钉死。
...
| 进程 | 角色 | 允许做什么 | 禁止做什么 |
|------|------|-----------|-----------|
| **Main Process**（Tauri v2 Rust 后端 / `src-tauri/`） | 唯一可信源 | SQLite 读写、子进程管理、文件系统重操作、凭证加解密、IPC handler 注册 | 渲染 DOM、直接操作 UI |
...
- **正例**：Renderer 调 `window.nativesAPI.db.get(key)` → IPC `db:get` → Tauri Rust command 执行 SQLite 查询。
...
服务层 (Service)       ← DB 访问、环境注入、模块管理、搜索
        ↓ 依赖
基础设施层 (Infrastructure) ← Tauri v2 Rust 后端、SQLite、HTTP server、IPC
```
...
- **规则**：上层**可以**依赖下层；下层**必须不**依赖上层；同层之间**应该**避免环形依赖。具体：
  - 基础设施层（`src-tauri/src/db.rs` 等）**禁止** import 任何 `src/app/` / `src/components/`。
  - 框架层（`src/lib/theme-engine.ts` 等）**禁止** import 具体业务组件。
  - 应用层组件**可以**调用框架层与服务层。
- **正例**：`FileBrowser.tsx`（应用层）→ `tauri-adapter.ts`（框架层）→ Tauri IPC 到 `src-tauri/src/commands/`（基础设施层）。
...
- **等级**：MUST
- **分类**：进程、安全
- **规则**：Main → Renderer 的下行**必须**用 Tauri event 广播（如 `db-state-changed`）；Renderer → Main 的上行**必须**用 Tauri command invoke 经 adapter 暴露的 API。**禁止**用 `executeJavaScript` 把代码注入 Renderer（无法被审计、绕过 adapter 隔离）。
