...
> **关联源文件**: `src/main/`、`electron/`、`src/app/`、`src/components/`

---

## 一、本篇要约束什么

Natives 是 Electron + Next.js + SQLite 的多进程、多层系统。最大的技术风险是**跨层越权调用**（如前端直连 SQLite、插件直接 require Node 模块）。本篇把进程边界、模块分层、依赖方向钉死。
...
| 进程 | 角色 | 允许做什么 | 禁止做什么 |
|------|------|-----------|-----------|
| **Main Process**（Electron 主进程 / `src/main/` + `electron/main.ts`） | 唯一可信源 | SQLite 读写、子进程管理、文件系统重操作、凭证加解密、IPC handler 注册 | 渲染 DOM、直接操作 UI |
...
- **检查方法**：
  - Renderer 中**禁止**出现 `better-sqlite3` / `node-pty` / `fs` 重操作 / `child_process` 的直接 import。
...
服务层 (Service)       ← DB 访问、环境注入、模块管理、搜索
        ↓ 依赖
基础设施层 (Infrastructure) ← Electron Main、SQLite、watchdog、IPC
```
...
- **等级**：MUST
- **分类**：进程、安全
- **规则**：Main → Renderer 的下行**必须**用 `webContents.send` / IPC 事件广播（如 `db-state-changed`）；Renderer → Main 的上行**必须**用 `ipcRenderer.invoke` 经 preload 暴露的 API。**禁止**用 `webContents.executeJavaScript` 把代码注入 Renderer（无法被审计、绕过 preload 隔离）。
- **为什么**：防线 4（DB 单向总线 + 状态广播）依赖显式 IPC；`executeJavaScript` 会破坏 `contextIsolation`。
- **检查方法**：`grep -r "executeJavaScript" src electron` 除受控注释外应为空。
...
- **正例**（已在 `electron/preload.ts` 落地）：`db:get`、`db:set`、`terminal:create`、`terminal:resize`、`module:install`、`env:setVariable`、`app:version`。
