...
### Natives 主产品 / 内置模块 / 托管包 / 内部模块

- **Natives 主产品**：用户安装和使用的唯一产品；不是某一个进程的别名。
- **内置模块（Built-in Module）**：归属于 Natives、随完整安装包一起安装/更新/修复的业务能力，如基金（2026-09-12 收敛后的正式语义；取代旧"子应用独立交付"表述）。避免称为独立桌面应用。
- **托管包（Managed Package）**：内置模块的版本化构建单位；包含该模块的 UI 与业务实现，随 Natives 完整产品版本交付，不独立发布、不独立更新、不单独下载安装。
- **内部模块（Internal Module）**：托管包内部的职责划分（如持仓、账本、净值、导入）；不拥有独立产品、安装或发布身份。
- **统一套件（Suite）**：一次交给用户安装的 Natives 完整产品版本组合，含主程序、主 Host、浏览器组件与全部内置模块；不是第二安装状态权威。更新只有"更新 Natives"一种语义。
- **首用初始化（First-use Initialization）**：首次打开内置模块时在当前用户权限下完成的数据初始化/迁移；不是安装，无代码下载。
- **启用（Enabled）**：用户允许运行该模块；与代码是否存在、当前是否运行分别记录。
- **本地工程验收 / 正式发布验收**：前者证明指定本机开发环境真实可用，后者另证明生产信任与发布条件；二者不可互相冒充。具体门槛见 ADR-0029 与实施方案。

### App / RuntimeSpec / RuntimeInstance / Surface
- **App**：在本次 Apps 范围内指 Natives 管理和启动的子应用实体。
- **RuntimeSpec**：如何运行的声明。
