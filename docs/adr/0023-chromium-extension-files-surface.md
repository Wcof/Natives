...
> **2026-09-09 注记（ADR-0027）**：Files 约束全部不变（EOF 2 秒退出、无常驻 SW、无通用路径/进程接口、Files 不扩权）。ADR-0027 为扩展内 `app.html` 的官方应用 sandbox iframe 与应用自有 loopback 接口新增**有界例外**；该例外不适用于 Files 页，也不给 Core 增加通用 HTTP 服务。
