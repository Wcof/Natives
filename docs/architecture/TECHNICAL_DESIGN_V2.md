...
## Theme Simplification (已完成)

### 二皮肤精简已落地

已在以下文件中完成主题精简（移除 `editorial`，`warm-archive` 重命名为 `frosted-jasmine`）：

1. `src/lib/theme-engine.ts` — 仅保留 `frosted-jasmine` + `terminal-volt` 二主题
2. `src/app/globals.css` — `[data-theme]` 选择器统一为二主题名
3. `src/components/shell/SettingsPage.tsx` — 主题选择器仅显示二主题
4. `src/components/shell/CommandPalette.tsx` — 命令面板仅显示二主题
5. 测试文件 — 更新到新主题 ID

### 确保 2 套主题完整覆盖

验证 `frosted-jasmine` 和 `terminal-volt` 的所有 CSS 变量都完整定义，包括：
- 基础色板（bg, bg-2, bg-3, panel, border, text, text-dim, text-faint, accent）
- 终端 ANSI 配色
- 字体配置
- 圆角、阴影
- `--vibe-*` 布局令牌（sidebar、toolbar、content-panel、btn 等）
