# Natives UI 优化计划 — 对齐 Natives2 交互模式

> 参考: `References/Natives2`
...
## A. Header 顶栏 — 对齐 Natives2 topbar

### 设计

Natives2 的 topbar 是三区布局：左侧 sidebar toggle、中间 breadcrumb（可滚动）、右侧操作按钮。Natives 目前没有全局顶栏，breadcrumb 和操作按钮散落在 FileToolbar 中。
...
1. **Electron 窗口拖拽**: header 设为 `-webkit-app-region: drag`，交互元素 exempt（`-webkit-app-region: no-drag`）— 参考 Natives2 `style.css:886-890`
...
## B. 存储占用透视 — 对齐 Natives2 diskPanel

### 设计

Natives2 的 `diskPanel()` 是一个 overlay dialog，展示当前目录下各项的磁盘占用，带比例条，可点击目录钻入。当前 Natives 已有 `DiskUsage.tsx` 组件（基础版）和 `disk-usage.ts` 后端，但没有作为独立 overlay 使用。

**方案**: 将 DiskUsage 升级为 overlay dialog 模式（对齐 Natives2），并增加入口：statusbar "占用透视" 链接 + 右键菜单 "磁盘占用透视"。
...
| 文件 | 变更 |
|------|------|
| `src/components/files/DiskUsagePanel.tsx` | **新建** — overlay dialog 版本，复刻 Natives2 `diskPanel()` |
...
## C. 右键菜单增强 — 对齐 Natives2 contextMenu

### 设计

当前 Natives 右键菜单只有基础项。对照 Natives2 缺少以下功能：

| Natives2 菜单项 | 适用对象 | Natives 现状 | 需要 |
|---------------|---------|-------------|------|
| 磁盘占用透视 | 目录 + 空白 | ❌ 无 | ✅ 新增 |
| 在终端打开 | 目录 | ❌ 无 | ✅ 新增 |
...
│  你的个人桌面                                                            │
├─────────────────────────────────────────────────────────────────────────┤
│  [📦 Skills]   [⚡ 活跃]      [☕ 吃灰]       [⚠️ 异常]       [📊 预算]  │
│     24/30        12            10            0          12.3k/15k       │
├─────────────────────────────────────────────────────────────────────────┤
│  Token Hero Card                                                        │
│  真实消耗 Tokens                                                         │
│  总请求数 | 总成本 | 新增输入 | 输出 | 缓存 | 命中率                        │
├─────────────────────────────────────────────────────────────────────────┤
│  Token 趋势图 (今日 / 7天 / 30天)                                         │
├─────────────────────────────────────────────────────────────────────────┤
│  Skills Panel                                                           │
...
├─────────────────────────────────────────────────────────────────────────┤
│  按模型统计                                                              │
│  模型名 | 请求数 | Tokens | 成本 | 平均成本                                  │
└─────────────────────────────────────────────────────────────────────────┘
```

### 加载策略（冷热三级缓存）

Dashboard 的性能优化采用三层缓存 + 冷热数据分离策略：

| 层级 | 位置 | TTL | 效果 |
|------|------|-----|------|
| **Session 缓存** | `page.tsx` 模块级 `Map<string, CacheEntry>` | 热: 60s / 冷: 120s | SPA 内切换页面不重请求 |
| **Rust 命令缓存** | `usage.rs` 内存 `usage_cache` | 30s | 连续 IPC 调用不重复读盘 |
| **SQLite 持久缓存** | `persist_usage_to_db` | 永久 | 重启后也有旧数据可展示 |

热数据（skills / moduleCount）：
- 轻量 IPC，60s TTL
- 挂载时立即拉取并渲染
- 前台展示 "…" 占位 → 数据就绪后替换

冷数据（usage）：
- 重 IO（读 `~/.claude/stats-cache.json` + 扫 Codex session 文件）
- 120s TTL，`Promise.resolve().then()` 延迟到下一帧加载
- 不阻塞初始渲染，后台慢慢就绪后更新相应组件

### 刷新按钮

- greeting 区右上角 `RefreshCw` 按钮
- 点击后 `invalidateCache()` 清空 Session 缓存
- 所有数据重新走完整 IPC 路径
- 按钮显示旋转动画（≥800ms）确保用户看到反馈

### 涉及文件（当前）

| 文件 | 变更说明 |
|------|---------|
| `src/app/page.tsx` | 三路并发+session cache+热冷分离+刷新按钮（319行） |
| `src/components/dashboard/TokenHero.tsx` | 用量主展示卡片 |
| `src/components/dashboard/TokenTrendChart.tsx` | 趋势图（recharts），useMemo 替代 useState+useEffect |
| `src/components/dashboard/SkillsPanel.tsx` | Skill 列表面板，useMemo 替代 useState+useEffect |
| `src/components/dashboard/ModelStatsTable.tsx` | 模型统计表格，useMemo 替代 useState+useEffect |
| `src/components/dashboard/StatCard` | 内联 React.memo 统计卡片 |
| `src/i18n/zh.ts` + `en.ts` | dashboard 域键（含 refresh / refreshing） |
│  ● 配置已更新  1d                               │
└─────────────────────────────────────────────────┘
```
...
1. **Skill 透视区**: 5 个 glassmorphic stat cards（包括：唯一/总Skills、45天内活跃数及触发次数、吃灰中数量、异常/有问题计数、Claude常驻预算条及字符数）。点击全部 Skills 卡片可直接跳转至已安装模块页面。
2. **用量分析区**: 复刻 Natives2 `usagePanel.bar()` 样式 — 4px 高进度条，85%+ 变红（`#d4453a`），token 三联（近5h/今日/本周），重置时间显示。/ 预算进度条会随占用比例变色。
3. **最近使用/文件/活动**: 保持现有逻辑
...
Natives2 的终端默认是收起的（HTML 中 `#terminal-panel` 带 `hidden` class，启动时不读取 `fb_term_open`）。
...
- Natives2 的行为是**永远不恢复**终端状态（每次启动都收起）
