# Blueprint: File_SidebarTree (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
侧栏目录树，快速入口/收藏文件夹项带展开箭头逐级懒加载子文件夹(只列文件夹)，点行跳转，当前目录高亮。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/sidebartree.rs`
...
### State Machine
```
File_SidebarTree:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_SidebarTree — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/sidebartree.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SidebarTree — 文件管理器数据模型
interface File_SidebarTreeParams {
  path: string;
  // 特征专属参数
}
interface File_SidebarTreeResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn sidebartree(params: ...) -> Result<File_SidebarTreeResult>
```
