# Blueprint: File_ScreenshotBus (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
截图直通车，主进程startShotWatch监听系统截屏落盘(识别截屏命名习惯)，等文件大小稳定再通知(waitStable轮询)，新截图右下角浮出直通卡(到终端喂给agent/收进素材/标注圈重点)，45秒自动消失。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/screenshotbus.rs`
...
### State Machine
```
File_ScreenshotBus:  idle → watching → debouncing → (notify | noise_discard) → watching
```
- `idle`: 初始状态
- `watching`: 文件系统监听中
...
### Algorithm
```
// File_ScreenshotBus — Rust notify crate + debounce + stat noise filter
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/screenshotbus.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ScreenshotBus — 文件管理器数据模型
interface File_ScreenshotBusParams {
  path: string;
  // 特征专属参数
}
interface File_ScreenshotBusResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn screenshotbus(params: ...) -> Result<File_ScreenshotBusResult>
```
