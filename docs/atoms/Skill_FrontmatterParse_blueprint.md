# Blueprint: Skill_FrontmatterParse (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
SKILL.md frontmatter解析(skillFrontmatter)，正则提取description字段，处理块标量(>-)和引号包裹，超240字符截断显示。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/frontmatterparse.rs`
...
### State Machine
```
Skill_FrontmatterParse:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Skill_FrontmatterParse — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/frontmatterparse.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_FrontmatterParse — 技能系统数据模型
interface Skill_FrontmatterParseResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn frontmatterparse(params: ...) -> Result<Skill_FrontmatterParseResult>
```
