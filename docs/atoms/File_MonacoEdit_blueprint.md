# Blueprint: File_MonacoEdit (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Monaco编辑器集成，语法高亮+Cmd+F查找替换+多光标，停笔自动落盘+撤销/重做图标+实时保存状态(1分钟内显秒/1小时内显分秒/再久显钟点)，未保存守卫堵死Esc旁路。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/monacoedit.rs`
...
### State Machine
```
File_MonacoEdit:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_MonacoEdit — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/monacoedit.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_MonacoEdit — 文件管理器数据模型
interface File_MonacoEditParams {
  path: string;
  // 特征专属参数
}
interface File_MonacoEditResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn monacoedit(params: ...) -> Result<File_MonacoEditResult>
```
