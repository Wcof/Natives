# Blueprint: Skill_Toggle (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Skills启停开关，停用等于移入_disabled/子目录(立即对模型不可见、不删文件、可逆)，不用官方skillOverrides(该配置在用户级有已知失效bug claude-code#50631)，软链接型skill先解析绝对目标再迁移避免相对链接断链。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/toggle.rs`
...
### State Machine
```
Skill_Toggle:  enabled → disabling → disabled → enabling → enabled
```
- `enabled`: 启用状态
...
### Algorithm
```
// Skill_Toggle — 启停切换（move into/out of _disabled/）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/toggle.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_Toggle — 技能系统数据模型
interface Skill_ToggleResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn toggle(params: ...) -> Result<Skill_ToggleResult>
```
