# Blueprint: File_BreadcrumbNav (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
面包屑路径导航实现，逐级可点、当前目录高亮、macOS根路径适配、窄宽时面包屑可滚动让位给操作按钮。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/breadcrumbnav.rs`
...
### State Machine
```
File_BreadcrumbNav:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_BreadcrumbNav — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/breadcrumbnav.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_BreadcrumbNav — 文件管理器数据模型
interface File_BreadcrumbNavParams {
  path: string;
  // 特征专属参数
}
interface File_BreadcrumbNavResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn breadcrumbnav(params: ...) -> Result<File_BreadcrumbNavResult>
```
