# Blueprint: Skill_Trash (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Skills卸载，移到系统废纸篓随时可恢复(skillTrash调trashPath)，validateSkillDir路径校验只允许动最近一次扫描出来的skill目录杜绝任意路径删除。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/trash.rs`
...
### State Machine
```
Skill_Trash:  idle → scanning → (parsing | validating) → (scanned | error) → idle
```
- `idle`: 初始
- `scanning`: 递归遍历目录
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/trash.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_Trash — 技能系统数据模型
interface Skill_TrashResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn trash(params: ...) -> Result<Skill_TrashResult>
```
