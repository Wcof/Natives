# Blueprint: Term_Maximize (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端铺满模式，双击顶栏空白等于终端挤掉文件区和预览铺满窗口，主动导航退出该状态让文件区回来。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/maximize.rs`
...
### State Machine
```
Term_Maximize:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/maximize.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_Maximize — 终端数据模型
interface Term_MaximizeParams {
  id: string;
  // 特征专属参数
}
interface Term_MaximizeResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn maximize(params: ...) -> Result<Term_MaximizeResult>
```
