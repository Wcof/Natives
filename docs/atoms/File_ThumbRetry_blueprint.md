# Blueprint: File_ThumbRetry (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
缩略图加载失败自动重试再优雅隐藏，Retina全屏截图较大写盘未完成时等文件大小稳定再通知(waitStable轮询)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/thumbretry.rs`
...
### State Machine
```
File_ThumbRetry:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/thumbretry.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ThumbRetry — 文件管理器数据模型
interface File_ThumbRetryParams {
  path: string;
  // 特征专属参数
}
interface File_ThumbRetryResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn thumbretry(params: ...) -> Result<File_ThumbRetryResult>
```
