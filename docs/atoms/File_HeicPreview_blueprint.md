# Blueprint: File_HeicPreview (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
的HEIC/HEIF预览实现，serveRaw遇heic用macOS sips转码jpeg并缓存(复用缩略图LRU)，三条路径透明覆盖，零新依赖。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/heicpreview.rs`
...
### State Machine
```
File_HeicPreview:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_HeicPreview — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/heicpreview.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_HeicPreview — 文件管理器数据模型
interface File_HeicPreviewParams {
  path: string;
  // 特征专属参数
}
interface File_HeicPreviewResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn heicpreview(params: ...) -> Result<File_HeicPreviewResult>
```
