# Blueprint: Term_CwdSync (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端cwd与文件树双向联动：点文件夹到终端cd过去(openTermPath)；终端里cd到左侧文件树跟着定位(locateCwd从lsof+cwd文件推断，中文目录名UTF-8 locale+解码兜底)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/cwdsync.rs`
...
### State Machine
```
Term_CwdSync:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_CwdSync — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/cwdsync.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_CwdSync — 终端数据模型
interface Term_CwdSyncParams {
  id: string;
  // 特征专属参数
}
interface Term_CwdSyncResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn cwdsync(params: ...) -> Result<Term_CwdSyncResult>
```
