# Blueprint: Term_WechatPersona (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信人格/目标/cwd设置(wechat:setPersona/setTarget/setCwd)，可切换agent人格+指定目标agent+指定工作目录，多项目多agent灵活切换。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatpersona.rs`
...
### State Machine
```
Term_WechatPersona:  enabled → disabling → disabled → enabling → enabled
```
- `enabled`: 启用状态
...
### Algorithm
```
// Term_WechatPersona — 启停切换（move into/out of _disabled/）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatpersona.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatPersona — 终端数据模型
interface Term_WechatPersonaParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatPersonaResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatpersona(params: ...) -> Result<Term_WechatPersonaResult>
```
