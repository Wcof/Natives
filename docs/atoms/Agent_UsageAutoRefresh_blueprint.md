# Blueprint: Agent_UsageAutoRefresh (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Agent用量面板自动刷新，展开时ensureStatusTick定时轮询，收起时停止，避免不必要网络请求。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/usageautorefresh.rs`
...
### State Machine
```
Agent_UsageAutoRefresh:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Agent_UsageAutoRefresh — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/usageautorefresh.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_UsageAutoRefresh — 用量统计数据模型
interface Agent_UsageAutoRefreshResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn usageautorefresh(params: ...) -> Result<Agent_UsageAutoRefreshResult>
```
