# Blueprint: Agent_CodexStaleGuard (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Codex配额快照过期守卫，capturedAt之后的窗口重置时间(resetsAt/resets_in_seconds/window_minutes)已过则usedPercent归零+stale标true，避免21小时前的57%失真数据误导。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/codexstaleguard.rs`
...
### State Machine
```
Agent_CodexStaleGuard:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Agent_CodexStaleGuard — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/codexstaleguard.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_CodexStaleGuard — 用量统计数据模型
interface Agent_CodexStaleGuardResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn codexstaleguard(params: ...) -> Result<Agent_CodexStaleGuardResult>
```
