# Blueprint: File_ScrollbarStyle (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
细圆角胶囊样式滚动条，带透明边距hover色跟随主题，不再是写死的深色粗条，提升视觉精致度。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/scrollbarstyle.rs`
...
### State Machine
```
File_ScrollbarStyle:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ScrollbarStyle — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/scrollbarstyle.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ScrollbarStyle — 文件管理器数据模型
interface File_ScrollbarStyleParams {
  path: string;
  // 特征专属参数
}
interface File_ScrollbarStyleResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn scrollbarstyle(params: ...) -> Result<File_ScrollbarStyleResult>
```
