# Blueprint: Skill_DescBudget (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Skills Context预算条，全局常驻description总量vs SKILL_BUDGET_CHARS预算估算线，超限红色警示(超出部分被模型静默丢弃)，项目级skill不计入常驻预算。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/descbudget.rs`
...
### State Machine
```
Skill_DescBudget:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Skill_DescBudget — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/descbudget.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_DescBudget — 技能系统数据模型
interface Skill_DescBudgetResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn descbudget(params: ...) -> Result<Skill_DescBudgetResult>
```
