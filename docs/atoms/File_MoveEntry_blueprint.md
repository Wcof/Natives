# Blueprint: File_MoveEntry (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
文件移动(movePath)和创建(createEntry)操作，移动含冲突检测+同名自动编号，创建支持文件和文件夹两种类型。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/moveentry.rs`
...
### State Machine
```
File_MoveEntry:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_MoveEntry — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/moveentry.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_MoveEntry — 文件管理器数据模型
interface File_MoveEntryParams {
  path: string;
  // 特征专属参数
}
interface File_MoveEntryResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn moveentry(params: ...) -> Result<File_MoveEntryResult>
```
