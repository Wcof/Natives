# Blueprint: Agent_ClaudeTokenStats (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Claude Code token统计(claudeUsage)，增量解析~/.claude/projects下jsonl会话日志(parseClaudeFile带offset增量+lastMsgId去重)，分近5h/今日/本周三档聚合，含input/output/cacheRead/cacheCreate分类统计。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/claudetokenstats.rs`
...
### State Machine
```
Agent_ClaudeTokenStats:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_ClaudeTokenStats — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/claudetokenstats.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_ClaudeTokenStats — 用量统计数据模型
interface Agent_ClaudeTokenStatsResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn claudetokenstats(params: ...) -> Result<Agent_ClaudeTokenStatsResult>
```
