# Blueprint: File_StatusBar (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
底部状态条，当前文件夹基础信息(N项+文件夹/文件数+文件合计大小)，可开合的Agent用量/项目记忆/占用透视入口。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/statusbar.rs`
...
### State Machine
```
File_StatusBar:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_StatusBar — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/statusbar.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_StatusBar — 文件管理器数据模型
interface File_StatusBarParams {
  path: string;
  // 特征专属参数
}
interface File_StatusBarResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn statusbar(params: ...) -> Result<File_StatusBarResult>
```
