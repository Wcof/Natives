# Blueprint: Agent_UsagePanel (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Agent用量面板，侧栏底部入口，展示Claude Code(近5h/今日/本周token+官方限额进度条)与Codex(5h窗口+周配额百分比+重置时间)，可开合，开着时每分钟自动刷新，30秒缓存。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/usagepanel.rs`
...
### State Machine
```
Agent_UsagePanel:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_UsagePanel — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/usagepanel.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_UsagePanel — 用量统计数据模型
interface Agent_UsagePanelResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn usagepanel(params: ...) -> Result<Agent_UsagePanelResult>
```
