# Blueprint: File_DotFileToggle (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
显示/隐藏点文件开关，以及隐藏文件(.env/.git配置等)和sqlite临时sidecar在文件跟随/变更收件箱中的噪声过滤。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/dotfiletoggle.rs`
...
### State Machine
```
File_DotFileToggle:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_DotFileToggle — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/dotfiletoggle.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_DotFileToggle — 文件管理器数据模型
interface File_DotFileToggleParams {
  path: string;
  // 特征专属参数
}
interface File_DotFileToggleResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn dotfiletoggle(params: ...) -> Result<File_DotFileToggleResult>
```
