# Blueprint: File_OneClickOpen (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
一键多方式打开：默认应用(open/start/xdg-open)、编辑器(code命令未装回退默认)、Finder显示(open -R)、复制路径、系统终端(open -a Terminal)，跨平台三路适配。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/oneclickopen.rs`
...
### State Machine
```
File_OneClickOpen:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_OneClickOpen — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/oneclickopen.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_OneClickOpen — 文件管理器数据模型
interface File_OneClickOpenParams {
  path: string;
  // 特征专属参数
}
interface File_OneClickOpenResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn oneclickopen(params: ...) -> Result<File_OneClickOpenResult>
```
