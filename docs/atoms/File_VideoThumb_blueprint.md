# Blueprint: File_VideoThumb (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
视频缩略图，用qlmanage QuickLook抽帧+黑帧onerror兜底，video标签内嵌播放支持Range流式。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/videothumb.rs`
...
### State Machine
```
File_VideoThumb:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/videothumb.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_VideoThumb — 文件管理器数据模型
interface File_VideoThumbParams {
  path: string;
  // 特征专属参数
}
interface File_VideoThumbResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn videothumb(params: ...) -> Result<File_VideoThumbResult>
```
