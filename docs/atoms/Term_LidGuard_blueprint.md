# Blueprint: Term_LidGuard (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
合盖保持运行，setLidIntent调pmset -b disablesleep 1，installSudoers配置免密码sudoers，refreshLidGuard刷新状态。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/lidguard.rs`
...
### State Machine
```
Term_LidGuard:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_LidGuard — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/lidguard.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_LidGuard — 终端数据模型
interface Term_LidGuardParams {
  id: string;
  // 特征专属参数
}
interface Term_LidGuardResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn lidguard(params: ...) -> Result<Term_LidGuardResult>
```
