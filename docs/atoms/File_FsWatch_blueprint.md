# Blueprint: File_FsWatch (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
的fs.watch递归文件监听，主进程转发事件前先stat检查mtime/ctime是否真正变更(丢弃atime/元数据噪声如LaunchServices的lastuseddate)，读文件从此安静，3秒内自己打开触发的伪事件按噪声丢弃。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/fswatch.rs`
...
### State Machine
```
File_FsWatch:  idle → watching → debouncing → (notify | noise_discard) → watching
```
- `idle`: 初始状态
- `watching`: 文件系统监听中
...
### Algorithm
```
// File_FsWatch — Rust notify crate + debounce + stat noise filter
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/fswatch.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FsWatch — 文件管理器数据模型
interface File_FsWatchParams {
  path: string;
  // 特征专属参数
}
interface File_FsWatchResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn fswatch(params: ...) -> Result<File_FsWatchResult>
```
