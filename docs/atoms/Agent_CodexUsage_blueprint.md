# Blueprint: Agent_CodexUsage (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Codex用量统计(codexUsage)，读~/.codex/sessions下rollout jsonl尾部抓最后一条带rate_limits的token_count(官方配额快照)，窗口重置后旧百分比归零标stale。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/codexusage.rs`
...
### State Machine
```
Agent_CodexUsage:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_CodexUsage — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/codexusage.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_CodexUsage — 用量统计数据模型
interface Agent_CodexUsageResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn codexusage(params: ...) -> Result<Agent_CodexUsageResult>
```
