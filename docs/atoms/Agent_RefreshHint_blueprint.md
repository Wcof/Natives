# Blueprint: Agent_RefreshHint (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Agent用量刷新状态提示，数据显示最后更新时间(ago方法)，官方限额拿不到时自动回退本地统计，给用户明确的数据来源指示。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/refreshhint.rs`
...
### State Machine
```
Agent_RefreshHint:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_RefreshHint — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/refreshhint.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_RefreshHint — 用量统计数据模型
interface Agent_RefreshHintResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn refreshhint(params: ...) -> Result<Agent_RefreshHintResult>
```
