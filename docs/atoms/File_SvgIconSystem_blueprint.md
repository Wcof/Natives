# Blueprint: File_SvgIconSystem (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
全套彩色矢量SVG图标系统，替换所有emoji(品牌logo/面包屑/空状态/收藏星标/预览动作/工具栏/HTML预览按钮)，richIcon按文件类型返回对应SVG，iconColorFor随皮肤变色。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/svgiconsystem.rs`
...
### State Machine
```
File_SvgIconSystem:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_SvgIconSystem — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/svgiconsystem.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SvgIconSystem — 文件管理器数据模型
interface File_SvgIconSystemParams {
  path: string;
  // 特征专属参数
}
interface File_SvgIconSystemResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn svgiconsystem(params: ...) -> Result<File_SvgIconSystemResult>
```
