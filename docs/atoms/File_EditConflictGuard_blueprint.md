# Blueprint: File_EditConflictGuard (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
编辑冲突保护，保存时检查expectedMtime，外部改盘或删除则抛conflict错误拒绝盲覆盖，前端弹确认。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/editconflictguard.rs`
...
### State Machine
```
File_EditConflictGuard:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_EditConflictGuard — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/editconflictguard.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_EditConflictGuard — 文件管理器数据模型
interface File_EditConflictGuardParams {
  path: string;
  // 特征专属参数
}
interface File_EditConflictGuardResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn editconflictguard(params: ...) -> Result<File_EditConflictGuardResult>
```
