# Blueprint: File_GridTypeTint (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
网格图标类型色磨砂圆角底座，类型色tint从13%加到20%提升辨识度，图标底座有圆角+半透明底色。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/gridtypetint.rs`
...
### State Machine
```
File_GridTypeTint:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_GridTypeTint — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/gridtypetint.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_GridTypeTint — 文件管理器数据模型
interface File_GridTypeTintParams {
  path: string;
  // 特征专属参数
}
interface File_GridTypeTintResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn gridtypetint(params: ...) -> Result<File_GridTypeTintResult>
```
