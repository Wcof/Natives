# Blueprint: Term_WechatConvMgmt (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信对话管理IPC，wechat:conversation读对话+wechat:newConversation开新对话+wechat:compact压缩上下文+wechat:check主动探活+wechat:send发送消息，完整对话生命周期。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatconvmgmt.rs`
...
### State Machine
```
Term_WechatConvMgmt:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_WechatConvMgmt — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatconvmgmt.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatConvMgmt — 终端数据模型
interface Term_WechatConvMgmtParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatConvMgmtResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatconvmgmt(params: ...) -> Result<Term_WechatConvMgmtResult>
```
