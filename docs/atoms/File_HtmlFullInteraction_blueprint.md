# Blueprint: File_HtmlFullInteraction (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
HTML预览支持完整交互：点击、键盘、localStorage、fetch、表单、SPA全部能跑，靠隔离预览源做到安全。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/htmlfullinteraction.rs`
...
### State Machine
```
File_HtmlFullInteraction:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_HtmlFullInteraction — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/htmlfullinteraction.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_HtmlFullInteraction — 文件管理器数据模型
interface File_HtmlFullInteractionParams {
  path: string;
  // 特征专属参数
}
interface File_HtmlFullInteractionResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn htmlfullinteraction(params: ...) -> Result<File_HtmlFullInteractionResult>
```
