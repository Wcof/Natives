# Blueprint: Term_OutputTail (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端输出尾巴保留(termTails Map)，每个终端保留最后约4KB去ANSI后的输出文本，给微信agent跨终端感知(手机上看电脑在跑啥/卡哪)和完成通知摘录用。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/outputtail.rs`
...
### State Machine
```
Term_OutputTail:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_OutputTail — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/outputtail.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_OutputTail — 终端数据模型
interface Term_OutputTailParams {
  id: string;
  // 特征专属参数
}
interface Term_OutputTailResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn outputtail(params: ...) -> Result<Term_OutputTailResult>
```
