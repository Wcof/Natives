# Blueprint: File_GridListView (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
文件管理器的网格/列表双视图切换机制，含状态记忆、缩略图三档尺寸(68/98/132列宽)、同屏文件数最大化策略。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/gridlistview.rs`
...
### State Machine
```
File_GridListView:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_GridListView — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/gridlistview.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_GridListView — 文件管理器数据模型
interface File_GridListViewParams {
  path: string;
  // 特征专属参数
}
interface File_GridListViewResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn gridlistview(params: ...) -> Result<File_GridListViewResult>
```
