# Blueprint: Term_RecordingPrune (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
录像自动裁剪(recPrune)，保留最近60个文件+总量800MB上限，超了从最旧删起(正在录的跳过)，启动时和每次新录制触发裁剪，防磁盘无限涨。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/recordingprune.rs`
...
### State Machine
```
Term_RecordingPrune:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_RecordingPrune — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/recordingprune.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_RecordingPrune — 终端数据模型
interface Term_RecordingPruneParams {
  id: string;
  // 特征专属参数
}
interface Term_RecordingPruneResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn recordingprune(params: ...) -> Result<Term_RecordingPruneResult>
```
