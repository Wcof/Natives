# Blueprint: Term_ResizeEvent (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端resize事件(pty:resize)，cols/rows变更时通知node-pty重排+记录resize事件到cast录像，TUI程序(vim/htop/claude code)重排正确不花屏。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/resizeevent.rs`
...
### State Machine
```
Term_ResizeEvent:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_ResizeEvent — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/resizeevent.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_ResizeEvent — 终端数据模型
interface Term_ResizeEventParams {
  id: string;
  // 特征专属参数
}
interface Term_ResizeEventResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn resizeevent(params: ...) -> Result<Term_ResizeEventResult>
```
