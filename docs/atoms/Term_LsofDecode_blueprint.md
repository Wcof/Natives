# Blueprint: Term_LsofDecode (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端cwd获取的lsof中文路径解码(decodeLsofPath)，GUI启动的app没有UTF-8 locale时lsof把中文按字节转义成\xNN字面量，调lsof时显式给LC_ALL=en_US.UTF-8+这里留一层\xNN解码兜底。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/lsofdecode.rs`
...
### State Machine
```
Term_LsofDecode:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_LsofDecode — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/lsofdecode.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_LsofDecode — 终端数据模型
interface Term_LsofDecodeParams {
  id: string;
  // 特征专属参数
}
interface Term_LsofDecodeResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn lsofdecode(params: ...) -> Result<Term_LsofDecodeResult>
```
