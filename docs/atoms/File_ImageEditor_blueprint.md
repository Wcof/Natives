# Blueprint: File_ImageEditor (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
图片编辑器：标注/打码/自由画笔/粗细可视化/原生取色器(自绘圆色点)/转格式/压缩/原生保存，进入编辑即全屏(CSS :has()感知DOM)，覆盖原图加确认(不可逆+有损警告)，ieUndo撤销栈+ieExport导出。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/imageeditor.rs`
...
### State Machine
```
File_ImageEditor:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_ImageEditor — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/imageeditor.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ImageEditor — 文件管理器数据模型
interface File_ImageEditorParams {
  path: string;
  // 特征专属参数
}
interface File_ImageEditorResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn imageeditor(params: ...) -> Result<File_ImageEditorResult>
```
