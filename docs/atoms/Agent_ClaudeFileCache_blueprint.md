# Blueprint: Agent_ClaudeFileCache (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Claude会话文件增量解析缓存(claudeFileCache Map)，offset追踪上次读到哪+lastMsgId去重同一消息多行落盘，文件被截断重写时自动重置，过期文件自动出缓存。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/claudefilecache.rs`
...
### State Machine
```
Agent_ClaudeFileCache:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Agent_ClaudeFileCache — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/claudefilecache.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_ClaudeFileCache — 用量统计数据模型
interface Agent_ClaudeFileCacheResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn claudefilecache(params: ...) -> Result<Agent_ClaudeFileCacheResult>
```
