# Blueprint: File_LargeFileLazy (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
大文件延迟加载策略，大于2MB文件仅读前256KB加UTF-8边界回退(末尾多字节字符不切坏)，显示截断提示，避免大文件阻塞渲染。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/largefilelazy.rs`
...
### State Machine
```
File_LargeFileLazy:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_LargeFileLazy — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/largefilelazy.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_LargeFileLazy — 文件管理器数据模型
interface File_LargeFileLazyParams {
  path: string;
  // 特征专属参数
}
interface File_LargeFileLazyResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn largefilelazy(params: ...) -> Result<File_LargeFileLazyResult>
```
