# Blueprint: Term_PathDrag (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端文件路径拖进文件区(flingToTerminal反向：文件拖进终端插入路径)，以及skill行拖进终端注入/skill-name。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/pathdrag.rs`
...
### State Machine
```
Term_PathDrag:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_PathDrag — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/pathdrag.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_PathDrag — 终端数据模型
interface Term_PathDragParams {
  id: string;
  // 特征专属参数
}
interface Term_PathDragResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn pathdrag(params: ...) -> Result<Term_PathDragResult>
```
