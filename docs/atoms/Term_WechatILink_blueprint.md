# Blueprint: Term_WechatILink (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信ClawBot v2架构(非OpenClaw)，走腾讯iLink协议直连+本机claude/codex无头实例(electron/wechat/bridge.js含ilink.js+driver.js)，绕开OpenClaw中间层，延迟更低+不依赖第三方网关。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatilink.rs`
...
### State Machine
```
Term_WechatILink:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_WechatILink — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatilink.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatILink — 终端数据模型
interface Term_WechatILinkParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatILinkResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatilink(params: ...) -> Result<Term_WechatILinkResult>
```
