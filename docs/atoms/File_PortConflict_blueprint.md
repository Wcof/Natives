# Blueprint: File_PortConflict (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
端口占用检测，server.js启动时EADDRINUSE错误给出中文提示(FanBox很可能已经在运行了+直接打开浏览器访问就行+想另开一个换端口FANBOX_PORT=8080)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/portconflict.rs`
...
### State Machine
```
File_PortConflict:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// File_PortConflict — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/portconflict.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_PortConflict — 文件管理器数据模型
interface File_PortConflictParams {
  path: string;
  // 特征专属参数
}
interface File_PortConflictResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn portconflict(params: ...) -> Result<File_PortConflictResult>
```
