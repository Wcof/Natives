# Blueprint: File_DefaultRoots (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
侧栏快速入口自动检测(defaultRoots)，候选主目录/桌面/文档/下载/Code/Projects/Developer，只显示实际存在的目录，跨平台适配。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/defaultroots.rs`
...
### State Machine
```
File_DefaultRoots:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_DefaultRoots — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/defaultroots.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_DefaultRoots — 文件管理器数据模型
interface File_DefaultRootsParams {
  path: string;
  // 特征专属参数
}
interface File_DefaultRootsResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn defaultroots(params: ...) -> Result<File_DefaultRootsResult>
```
