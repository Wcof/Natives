# Blueprint: Agent_ClaudeOfficialLimits (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Claude Code官方限额查询(claudeOfficialLimits)，用OAuth token调api.anthropic.com/api/oauth/usage，返回和/usage同源的5h窗口+7天配额百分比与重置时间，这是fanbox唯一出网请求。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/claudeofficiallimits.rs`
...
### State Machine
```
Agent_ClaudeOfficialLimits:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_ClaudeOfficialLimits — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/claudeofficiallimits.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_ClaudeOfficialLimits — 用量统计数据模型
interface Agent_ClaudeOfficialLimitsResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn claudeofficiallimits(params: ...) -> Result<Agent_ClaudeOfficialLimitsResult>
```
