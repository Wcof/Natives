# Blueprint: File_DragDrop (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
拖拽存盘体系：Finder文件(有路径copyInto)、截图浮窗(无路径saveTemp到saveInto)、内部img拖入文件区存盘，同名不覆盖仿访达foo 2.png，drop区撑满文件夹高度，dropIn动画+edIn淡入。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/dragdrop.rs`
...
### State Machine
```
File_DragDrop:  idle → hover → dragging → (drop | cancel) → (saving | copying) → done
```
- `idle`: 初始
- `hover`: 拖拽物悬浮在目标区
- `dragging`: 拖拽中
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/dragdrop.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_DragDrop — 文件管理器数据模型
interface File_DragDropParams {
  path: string;
  // 特征专属参数
}
interface File_DragDropResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn dragdrop(params: ...) -> Result<File_DragDropResult>
```
