# Blueprint: File_AgentProjects (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
侧栏Agent项目列表，自动扫Claude Code和Codex本机会话日志，列出最近30天被coding agent处理过的项目文件夹(agent圆点+活跃时间)，readCwdFromHead从JSONL头部提取真实cwd。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/agentprojects.rs`
...
### State Machine
```
File_AgentProjects:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_AgentProjects — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/agentprojects.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_AgentProjects — 文件管理器数据模型
interface File_AgentProjectsParams {
  path: string;
  // 特征专属参数
}
interface File_AgentProjectsResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn agentprojects(params: ...) -> Result<File_AgentProjectsResult>
```
