# Blueprint: File_ThumbGen (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
缩略图生成与缓存体系，图片走sips缩放+视频/PDF走qlmanage QuickLook抽帧，400MB LRU上限自动裁剪(pruneThumbs)，透明格式出PNG保留alpha+非透明出JPEG省空间，并发生成去重(thumbInflight Map)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/thumbgen.rs`
...
### State Machine
```
File_ThumbGen:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/thumbgen.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ThumbGen — 文件管理器数据模型
interface File_ThumbGenParams {
  path: string;
  // 特征专属参数
}
interface File_ThumbGenResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn thumbgen(params: ...) -> Result<File_ThumbGenResult>
```
