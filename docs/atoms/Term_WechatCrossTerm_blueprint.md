# Blueprint: Term_WechatCrossTerm (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信跨终端感知与控制(termControl)，微信agent可list所有终端(id/cwd/进程名/忙碌状态/最近输出尾巴)+向指定终端send文本(遥控)，手机上看电脑在跑啥并能远程操控。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatcrossterm.rs`
...
### State Machine
```
Term_WechatCrossTerm:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_WechatCrossTerm — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatcrossterm.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatCrossTerm — 终端数据模型
interface Term_WechatCrossTermParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatCrossTermResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatcrossterm(params: ...) -> Result<Term_WechatCrossTermResult>
```
