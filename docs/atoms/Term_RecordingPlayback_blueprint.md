# Blueprint: Term_RecordingPlayback (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
录像回放，用与live终端完全相同的xterm配置逐像素一致，时间压缩压等待不压输出(agent思考长静默封顶，流式节奏原样保留)，2小时可压到约1分钟。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/recordingplayback.rs`
...
### State Machine
```
Term_RecordingPlayback:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_RecordingPlayback — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/recordingplayback.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_RecordingPlayback — 终端数据模型
interface Term_RecordingPlaybackParams {
  id: string;
  // 特征专属参数
}
interface Term_RecordingPlaybackResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn recordingplayback(params: ...) -> Result<Term_RecordingPlaybackResult>
```
