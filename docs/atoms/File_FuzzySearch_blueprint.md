# Blueprint: File_FuzzySearch (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
的Cmd+K全局模糊搜索，子序列+连续/词首/靠前加权打分算法(fuzzyScore:基础10分+连续streak*8+词首+15+靠前8-ti*0.1)，命中字符高亮渲染(hlFuzzy)，全机/当前目录Tab切换。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/fuzzysearch.rs`
...
### State Machine
```
File_FuzzySearch:  idle → typing → (debounce) → searching → (results | no_results) → idle
```
- `idle`: 搜索栏空闲
- `typing`: 用户输入
- `debounce`: 节流等待（防高频触发）
...
### Algorithm
```
// File_FuzzySearch — 异步搜索（fuzzyScore / grep / Spotlight）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/fuzzysearch.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FuzzySearch — 文件管理器数据模型
interface File_FuzzySearchParams {
  path: string;
  // 特征专属参数
}
interface File_FuzzySearchResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn fuzzysearch(params: ...) -> Result<File_FuzzySearchResult>
```
