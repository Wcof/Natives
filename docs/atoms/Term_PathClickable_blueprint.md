# Blueprint: Term_PathClickable (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端路径可点击打开，宽进严出策略：含斜杠token候选到stat验证到验证得才划线；裸文件名按扩展名白名单免验证直接划线；全角胶水标点(：；，。、？！)进token切断字符集防误报；目录结尾斜杠也享受同等兜底。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/pathclickable.rs`
...
### State Machine
```
Term_PathClickable:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Term_PathClickable — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/pathclickable.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_PathClickable — 终端数据模型
interface Term_PathClickableParams {
  id: string;
  // 特征专属参数
}
interface Term_PathClickableResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn pathclickable(params: ...) -> Result<Term_PathClickableResult>
```
