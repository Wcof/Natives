# Blueprint: File_ChangeBadge (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
文件变更徽标系统，agent改文件即亮改N计数+tooltip定位深层改动，单一清理定时器防泄漏，isNoisyChange过滤二进制/临时文件噪声。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/changebadge.rs`
...
### State Machine
```
File_ChangeBadge:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ChangeBadge — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/changebadge.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ChangeBadge — 文件管理器数据模型
interface File_ChangeBadgeParams {
  path: string;
  // 特征专属参数
}
interface File_ChangeBadgeResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn changebadge(params: ...) -> Result<File_ChangeBadgeResult>
```
