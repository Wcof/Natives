# Blueprint: File_EnvPort (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
环境变量FANBOX_PORT自定义端口+FANBOX_NO_OPEN不自动开浏览器，支持开发/多实例场景。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/envport.rs`
...
### State Machine
```
File_EnvPort:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_EnvPort — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/envport.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_EnvPort — 文件管理器数据模型
interface File_EnvPortParams {
  path: string;
  // 特征专属参数
}
interface File_EnvPortResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn envport(params: ...) -> Result<File_EnvPortResult>
```
