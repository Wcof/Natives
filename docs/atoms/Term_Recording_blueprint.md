# Blueprint: Term_Recording (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端录像回放(黑匣子)，常开录制把PTY字节流旁路成asciinema v2 .cast(异步写盘失败静默自废对终端零侵入)，recStart/recEvent/recStop事件记录。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/recording.rs`
...
### State Machine
```
Term_Recording:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_Recording — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/recording.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_Recording — 终端数据模型
interface Term_RecordingParams {
  id: string;
  // 特征专属参数
}
interface Term_RecordingResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn recording(params: ...) -> Result<Term_RecordingResult>
```
