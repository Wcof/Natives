# Blueprint: Term_WechatAgentRegistry (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
可连接agent注册表(CONNECTABLE_AGENTS)，静态配置Claude Code/Codex/Kimi/Hermes等，新增agent只需注册表加一行+OpenClaw配置加一段runtime，UI/IPC/弹窗逻辑不动。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatagentregistry.rs`
...
### State Machine
```
Term_WechatAgentRegistry:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_WechatAgentRegistry — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatagentregistry.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatAgentRegistry — 终端数据模型
interface Term_WechatAgentRegistryParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatAgentRegistryResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatagentregistry(params: ...) -> Result<Term_WechatAgentRegistryResult>
```
