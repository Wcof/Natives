# Blueprint: File_RichPreview (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
原地验货预览面板：文本/代码(hljs语法高亮)、Markdown(marked渲染)、HTML(iframe实时渲染+源码切换)、图片(内嵌大图+棋盘格透明底)、视频/音频(Range流式播放)、PDF(iframe内嵌)、CSV/TSV(表格化渲染)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/richpreview.rs`
...
### State Machine
```
File_RichPreview:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_RichPreview — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/richpreview.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_RichPreview — 文件管理器数据模型
interface File_RichPreviewParams {
  path: string;
  // 特征专属参数
}
interface File_RichPreviewResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn richpreview(params: ...) -> Result<File_RichPreviewResult>
```
