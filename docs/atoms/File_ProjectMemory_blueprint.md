# Blueprint: File_ProjectMemory (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
项目记忆面板，当前文件夹的agent会话考古——历史会话列表(首条消息当标题)、每次会话改过的文件(点击直达)、触发过的skill，续上一键在内嵌终端claude --resume/codex resume接上当时上下文，数据源本地会话日志按文件缓存增量解析。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/projectmemory.rs`
...
### State Machine
```
File_ProjectMemory:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ProjectMemory — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/projectmemory.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ProjectMemory — 文件管理器数据模型
interface File_ProjectMemoryParams {
  path: string;
  // 特征专属参数
}
interface File_ProjectMemoryResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn projectmemory(params: ...) -> Result<File_ProjectMemoryResult>
```
