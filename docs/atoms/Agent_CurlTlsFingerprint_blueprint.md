# Blueprint: Agent_CurlTlsFingerprint (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
调用官方限额接口走系统curl而非Node https的原因：该接口按TLS指纹拦——同样请求头curl能200、Node直接403，token经stdin的curl配置传入不暴露在进程列表里。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/curltlsfingerprint.rs`
...
### State Machine
```
Agent_CurlTlsFingerprint:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_CurlTlsFingerprint — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/curltlsfingerprint.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_CurlTlsFingerprint — 用量统计数据模型
interface Agent_CurlTlsFingerprintResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn curltlsfingerprint(params: ...) -> Result<Agent_CurlTlsFingerprintResult>
```
