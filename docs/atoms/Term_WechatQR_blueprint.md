# Blueprint: Term_WechatQR (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信二维码渲染，捕获openclaw channels login的stdout，正则提取liteapp.weixin.qq.com URL，用JS QR库(qrcode npm)重渲染清晰二维码，降级方案等宽字体渲染字符画。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatqr.rs`
...
### State Machine
```
Term_WechatQR:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatqr.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatQR — 终端数据模型
interface Term_WechatQRParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatQRResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatqr(params: ...) -> Result<Term_WechatQRResult>
```
