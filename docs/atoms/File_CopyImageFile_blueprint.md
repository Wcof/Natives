# Blueprint: File_CopyImageFile (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
复制图片/文件到剪贴板(fanboxClipboard)，clip:image调Electron nativeImage写剪贴板，clip:file调shell脚本pbcopy，预览操作区一键复制。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/copyimagefile.rs`
...
### State Machine
```
File_CopyImageFile:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_CopyImageFile — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/copyimagefile.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_CopyImageFile — 文件管理器数据模型
interface File_CopyImageFileParams {
  path: string;
  // 特征专属参数
}
interface File_CopyImageFileResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn copyimagefile(params: ...) -> Result<File_CopyImageFileResult>
```
