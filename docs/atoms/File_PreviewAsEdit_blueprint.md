# Blueprint: File_PreviewAsEdit (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
预览即编辑设计，代码/纯文本一进预览区就是可编辑态(和md一致)，不用再点编辑按钮，html(看的是渲染形态)/csv/tsv(表格视图)保持只读展示，降低操作步数。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/previewasedit.rs`
...
### State Machine
```
File_PreviewAsEdit:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_PreviewAsEdit — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/previewasedit.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_PreviewAsEdit — 文件管理器数据模型
interface File_PreviewAsEditParams {
  path: string;
  // 特征专属参数
}
interface File_PreviewAsEditResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn previewasedit(params: ...) -> Result<File_PreviewAsEditResult>
```
