# Blueprint: Term_MultiTab (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端多标签页，每个项目一个agent session常驻不丢，标签栏渲染(renderTabs)+切换(activate)+关闭(closeTab)，关闭时kill PTY防进程泄漏，标签显示cwd目录名+agent状态圆点+忙碌色。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/multitab.rs`
...
### State Machine
```
Term_MultiTab:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/multitab.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_MultiTab — 终端数据模型
interface Term_MultiTabParams {
  id: string;
  // 特征专属参数
}
interface Term_MultiTabResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn multitab(params: ...) -> Result<Term_MultiTabResult>
```
