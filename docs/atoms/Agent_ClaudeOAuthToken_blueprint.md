# Blueprint: Agent_ClaudeOAuthToken (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Claude Code OAuth凭证获取(claudeOAuthToken)，macOS从Keychain读(security find-generic-password)，其他平台读~/.claude/.credentials.json，验expiresAt过期。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/claudeoauthtoken.rs`
...
### State Machine
```
Agent_ClaudeOAuthToken:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_ClaudeOAuthToken — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/claudeoauthtoken.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_ClaudeOAuthToken — 用量统计数据模型
interface Agent_ClaudeOAuthTokenResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn claudeoauthtoken(params: ...) -> Result<Agent_ClaudeOAuthTokenResult>
```
