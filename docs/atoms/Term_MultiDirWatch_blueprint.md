# Blueprint: Term_MultiDirWatch (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
多目录监听增量diff(fs:watch-set)，浏览目录+每个终端会话所在的项目目录同时监听，前端发来期望监听集后做增量diff(关掉多余+补上新增)，一下午开多个项目跑agent时不在线的项目也能感知变更。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/multidirwatch.rs`
...
### State Machine
```
Term_MultiDirWatch:  idle → watching → debouncing → (notify | noise_discard) → watching
```
- `idle`: 初始状态
- `watching`: 文件系统监听中
...
### Algorithm
```
// Term_MultiDirWatch — Rust notify crate + debounce + stat noise filter
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/multidirwatch.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_MultiDirWatch — 终端数据模型
interface Term_MultiDirWatchParams {
  id: string;
  // 特征专属参数
}
interface Term_MultiDirWatchResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn multidirwatch(params: ...) -> Result<Term_MultiDirWatchResult>
```
