# Blueprint: File_HtmlLocalImageFixup (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
HTML预览的本地图片裂图修复，file://绝对URL主动改写、其余绝对路径加载失败时兜底重写到/fs/镜像端点(对本来能加载的相对/远程/data引用零影响)，覆盖src/href/poster和内联style的background url。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/htmllocalimagefixup.rs`
...
### State Machine
```
File_HtmlLocalImageFixup:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_HtmlLocalImageFixup — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/htmllocalimagefixup.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_HtmlLocalImageFixup — 文件管理器数据模型
interface File_HtmlLocalImageFixupParams {
  path: string;
  // 特征专属参数
}
interface File_HtmlLocalImageFixupResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn htmllocalimagefixup(params: ...) -> Result<File_HtmlLocalImageFixupResult>
```
