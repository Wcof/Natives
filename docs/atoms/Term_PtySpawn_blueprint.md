# Blueprint: Term_PtySpawn (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端核心，Electron主进程node-pty起真PTY，渲染进程xterm.js渲染，IPC通道pty:spawn/input/resize/kill/cwd/proc，每终端独立进程，contextBridge安全桥接。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/ptyspawn.rs`
...
### State Machine
```
Term_PtySpawn:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_PtySpawn — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/ptyspawn.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_PtySpawn — 终端数据模型
interface Term_PtySpawnParams {
  id: string;
  // 特征专属参数
}
interface Term_PtySpawnResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn ptyspawn(params: ...) -> Result<Term_PtySpawnResult>
```
