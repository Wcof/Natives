# Blueprint: File_FollowMode (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
文件跟随的核心机制：绑定终端tab(钉死归属)，只跟该项目cwd下写入；节流窗口内看头优先级html/md大于代码大于其它；agent多文件快写不疯狂跳屏；手动浏览/编辑等于接管跟随自动停；开跟随先跟上最近一次变更不用干等下一笔。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/followmode.rs`
...
### State Machine
```
File_FollowMode:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_FollowMode — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/followmode.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FollowMode — 文件管理器数据模型
interface File_FollowModeParams {
  path: string;
  // 特征专属参数
}
interface File_FollowModeResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn followmode(params: ...) -> Result<File_FollowModeResult>
```
