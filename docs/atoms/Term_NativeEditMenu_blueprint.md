# Blueprint: Term_NativeEditMenu (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
原生Edit菜单(buildMenu)，包含撤销/重做/剪切/复制/粘贴/全选，关键是Cmd+C/Cmd+V在终端里生效(Electron默认不拦截需role:editMenu)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/nativeeditmenu.rs`
...
### State Machine
```
Term_NativeEditMenu:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// Term_NativeEditMenu — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/nativeeditmenu.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_NativeEditMenu — 终端数据模型
interface Term_NativeEditMenuParams {
  id: string;
  // 特征专属参数
}
interface Term_NativeEditMenuResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn nativeeditmenu(params: ...) -> Result<Term_NativeEditMenuResult>
```
