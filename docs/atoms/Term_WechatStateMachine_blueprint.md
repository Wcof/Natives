# Blueprint: Term_WechatStateMachine (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信连接弹窗状态机：未装OpenClaw(状态A引导安装)到装了网关没起(状态B一键启动)到网关在线(状态C选agent)到生成二维码(状态D)到展示二维码(状态E)到扫码成功(状态F已连接)，二维码过期自动刷新。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatstatemachine.rs`
...
### State Machine
```
Term_WechatStateMachine:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_WechatStateMachine — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatstatemachine.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatStateMachine — 终端数据模型
interface Term_WechatStateMachineParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatStateMachineResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatstatemachine(params: ...) -> Result<Term_WechatStateMachineResult>
```
