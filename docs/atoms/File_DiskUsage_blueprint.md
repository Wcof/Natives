# Blueprint: File_DiskUsage (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
磁盘占用透视，du口径的真实占用条形榜，目录可下钻可回上级，解电脑空间又满了。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/diskusage.rs`
...
### State Machine
```
File_DiskUsage:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_DiskUsage — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/diskusage.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_DiskUsage — 文件管理器数据模型
interface File_DiskUsageParams {
  path: string;
  // 特征专属参数
}
interface File_DiskUsageResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn diskusage(params: ...) -> Result<File_DiskUsageResult>
```
