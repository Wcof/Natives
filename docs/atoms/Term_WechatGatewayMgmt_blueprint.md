# Blueprint: Term_WechatGatewayMgmt (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
OpenClaw网关管理，检测安装状态(ocEnv)+拉起网关(ocRun)+登录后必须stop再start(不用restart因launchctl报I/O)+token存本地+手机登出即断连。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatgatewaymgmt.rs`
...
### State Machine
```
Term_WechatGatewayMgmt:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Term_WechatGatewayMgmt — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatgatewaymgmt.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatGatewayMgmt — 终端数据模型
interface Term_WechatGatewayMgmtParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatGatewayMgmtResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatgatewaymgmt(params: ...) -> Result<Term_WechatGatewayMgmtResult>
```
