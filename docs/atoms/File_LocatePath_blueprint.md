# Blueprint: File_LocatePath (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
路径定位系统(locatePath)，支持模糊匹配+多根搜索+tail后缀探测+alt别名+roots多根，statWithTail尝试多种路径变体找到真实文件。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/locatepath.rs`
...
### State Machine
```
File_LocatePath:  idle → typing → (debounce) → searching → (results | no_results) → idle
```
- `idle`: 搜索栏空闲
- `typing`: 用户输入
- `debounce`: 节流等待（防高频触发）
...
### Algorithm
```
// File_LocatePath — 异步搜索（fuzzyScore / grep / Spotlight）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/locatepath.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_LocatePath — 文件管理器数据模型
interface File_LocatePathParams {
  path: string;
  // 特征专属参数
}
interface File_LocatePathResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn locatepath(params: ...) -> Result<File_LocatePathResult>
```
