# Blueprint: Term_TermVerify (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端验证接口(/api/term-verify)，验证指定目录能否打开终端(权限+存在性)，前端据此决定终端按钮状态。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/termverify.rs`
...
### State Machine
```
Term_TermVerify:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_TermVerify — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/termverify.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_TermVerify — 终端数据模型
interface Term_TermVerifyParams {
  id: string;
  // 特征专属参数
}
interface Term_TermVerifyResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn termverify(params: ...) -> Result<Term_TermVerifyResult>
```
