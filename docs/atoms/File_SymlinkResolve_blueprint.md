# Blueprint: File_SymlinkResolve (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
符号链接处理，listDir中isSymbolicLink跟随stat解析真实类型，scanSkillRoot中软链跟随解析(skills.sh安装器常用相对路径软链)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/symlinkresolve.rs`
...
### State Machine
```
File_SymlinkResolve:  idle → scanning → (parsing | validating) → (scanned | error) → idle
```
- `idle`: 初始
- `scanning`: 递归遍历目录
...
### Algorithm
```
// File_SymlinkResolve — 递归扫描 + 软链跟随 + 健康检查
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/symlinkresolve.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SymlinkResolve — 文件管理器数据模型
interface File_SymlinkResolveParams {
  path: string;
  // 特征专属参数
}
interface File_SymlinkResolveResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn symlinkresolve(params: ...) -> Result<File_SymlinkResolveResult>
```
