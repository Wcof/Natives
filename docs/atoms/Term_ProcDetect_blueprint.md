# Blueprint: Term_ProcDetect (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端前台进程名检测(pty:proc)，node-pty维护的process属性返回当前前台进程名，用于判断是裸shell(zsh/bash/sh/fish/login)还是正跑着claude/codex等程序，驱动Agent状态显示+启动按钮行为。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/procdetect.rs`
...
### State Machine
```
Term_ProcDetect:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_ProcDetect — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/procdetect.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_ProcDetect — 终端数据模型
interface Term_ProcDetectParams {
  id: string;
  // 特征专属参数
}
interface Term_ProcDetectResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn procdetect(params: ...) -> Result<Term_ProcDetectResult>
```
