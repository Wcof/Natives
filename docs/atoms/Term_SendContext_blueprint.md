# Blueprint: Term_SendContext (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端向agent发送上下文信息(sendContext)，写入cwd路径等环境信息辅助agent定位。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/sendcontext.rs`
...
### State Machine
```
Term_SendContext:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_SendContext — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/sendcontext.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_SendContext — 终端数据模型
interface Term_SendContextParams {
  id: string;
  // 特征专属参数
}
interface Term_SendContextResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn sendcontext(params: ...) -> Result<Term_SendContextResult>
```
