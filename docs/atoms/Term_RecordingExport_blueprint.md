# Blueprint: Term_RecordingExport (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
录像导出，本机ffmpeg转MP4/GIF，检测不到ffmpeg优雅退回WebM(asciinema2png渲染+ffmpeg编码)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/recordingexport.rs`
...
### State Machine
```
Term_RecordingExport:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// Term_RecordingExport — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/recordingexport.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_RecordingExport — 终端数据模型
interface Term_RecordingExportParams {
  id: string;
  // 特征专属参数
}
interface Term_RecordingExportResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn recordingexport(params: ...) -> Result<Term_RecordingExportResult>
```
