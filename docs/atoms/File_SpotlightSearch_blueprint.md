# Blueprint: File_SpotlightSearch (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
如何白嫖macOS Spotlight索引(mdfind)做内容搜索，kMDItemTextContent属性查询+CJK子串匹配稳+忽略大小写/音调，覆盖全文+PDF/docx+截图OCR文字毫秒级返回。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/spotlightsearch.rs`
...
### State Machine
```
File_SpotlightSearch:  idle → typing → (debounce) → searching → (results | no_results) → idle
```
- `idle`: 搜索栏空闲
- `typing`: 用户输入
- `debounce`: 节流等待（防高频触发）
...
### Algorithm
```
// File_SpotlightSearch — 异步搜索（fuzzyScore / grep / Spotlight）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/spotlightsearch.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SpotlightSearch — 文件管理器数据模型
interface File_SpotlightSearchParams {
  path: string;
  // 特征专属参数
}
interface File_SpotlightSearchResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn spotlightsearch(params: ...) -> Result<File_SpotlightSearchResult>
```
