# Blueprint: File_GrepContent (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
的内容前缀全文搜索(grepFiles)，递归walk遍历带忽略表+结果上限12000+时间预算1.8s，按mtime倒序读文件，每文件最多4条命中+行号+200字符预览，总结果上限50+3.5s deadline。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/grepcontent.rs`
...
### State Machine
```
File_GrepContent:  idle → typing → (debounce) → searching → (results | no_results) → idle
```
- `idle`: 搜索栏空闲
- `typing`: 用户输入
- `debounce`: 节流等待（防高频触发）
...
### Algorithm
```
// File_GrepContent — 异步搜索（fuzzyScore / grep / Spotlight）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/grepcontent.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_GrepContent — 文件管理器数据模型
interface File_GrepContentParams {
  path: string;
  // 特征专属参数
}
interface File_GrepContentResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn grepcontent(params: ...) -> Result<File_GrepContentResult>
```
