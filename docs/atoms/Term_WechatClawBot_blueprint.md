# Blueprint: Term_WechatClawBot (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信ClawBot集成，顶栏入口到连接弹窗状态机到OpenClaw网关到扫码登录到微信驱动本机agent，IPC通道wechat:env/login/disconnect/cancel/sessions/transcript。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatclawbot.rs`
...
### State Machine
```
Term_WechatClawBot:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_WechatClawBot — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatclawbot.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatClawBot — 终端数据模型
interface Term_WechatClawBotParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatClawBotResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatclawbot(params: ...) -> Result<Term_WechatClawBotResult>
```
