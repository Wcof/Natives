# Blueprint: Term_NerdFont (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端字体优先使用系统已装Nerd Font(JetBrainsMono/MesloLGS/FiraCode/Hack/Symbols Nerd Font)，Starship/powerline主题图标正常显示不再是方块tofu。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/nerdfont.rs`
...
### State Machine
```
Term_NerdFont:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_NerdFont — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/nerdfont.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_NerdFont — 终端数据模型
interface Term_NerdFontParams {
  id: string;
  // 特征专属参数
}
interface Term_NerdFontResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn nerdfont(params: ...) -> Result<Term_NerdFontResult>
```
