# Blueprint: Term_LoginShell (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端用login shell启动(zsh -l)，读取~/.zprofile/~/.zlogin里配的PATH，解决GUI app找不到claude命令问题。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/loginshell.rs`
...
### State Machine
```
Term_LoginShell:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_LoginShell — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/loginshell.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_LoginShell — 终端数据模型
interface Term_LoginShellParams {
  id: string;
  // 特征专属参数
}
interface Term_LoginShellResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn loginshell(params: ...) -> Result<Term_LoginShellResult>
```
