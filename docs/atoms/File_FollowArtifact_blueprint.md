# Blueprint: File_FollowArtifact (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
跟随产物卡片，二进制/压缩包/安装包不再当文本跟随显示无法预览，改成干净卡片(图标+agent刚生成+大小+在Finder显示)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/followartifact.rs`
...
### State Machine
```
File_FollowArtifact:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_FollowArtifact — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/followartifact.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FollowArtifact — 文件管理器数据模型
interface File_FollowArtifactParams {
  path: string;
  // 特征专属参数
}
interface File_FollowArtifactResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn followartifact(params: ...) -> Result<File_FollowArtifactResult>
```
