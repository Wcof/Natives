# Blueprint: Term_AgentFalseComplete (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Agent误完成守卫，agent阶段性收工但底部还挂后台任务(1 shell, 1 monitor still running)时不弹通知，等真正收工再报。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/agentfalsecomplete.rs`
...
### State Machine
```
Term_AgentFalseComplete:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_AgentFalseComplete — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/agentfalsecomplete.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_AgentFalseComplete — 终端数据模型
interface Term_AgentFalseCompleteParams {
  id: string;
  // 特征专属参数
}
interface Term_AgentFalseCompleteResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn agentfalsecomplete(params: ...) -> Result<Term_AgentFalseCompleteResult>
```
