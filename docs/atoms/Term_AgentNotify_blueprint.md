# Blueprint: Term_AgentNotify (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Agent完成通知，正文带最后回复摘录(从终端缓冲区剥掉TUI框线/页脚捞正文)，标题带项目名，点通知拉回前台并切到对应标签(fanboxWin.focus)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/agentnotify.rs`
...
### State Machine
```
Term_AgentNotify:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_AgentNotify — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/agentnotify.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_AgentNotify — 终端数据模型
interface Term_AgentNotifyParams {
  id: string;
  // 特征专属参数
}
interface Term_AgentNotifyResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn agentnotify(params: ...) -> Result<Term_AgentNotifyResult>
```
