# Blueprint: File_FollowNoiseFilter (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
跟随噪声过滤四层：atime/元数据事件stat判否丢弃；隐藏文件+sqlite sidecar+tmp文件统一进噪声名单；跟随范围绑定终端tab的cwd不跟别的agent；节流窗口内看头优先级防低优顶掉高优。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/follownoisefilter.rs`
...
### State Machine
```
File_FollowNoiseFilter:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_FollowNoiseFilter — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/follownoisefilter.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FollowNoiseFilter — 文件管理器数据模型
interface File_FollowNoiseFilterParams {
  path: string;
  // 特征专属参数
}
interface File_FollowNoiseFilterResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn follownoisefilter(params: ...) -> Result<File_FollowNoiseFilterResult>
```
