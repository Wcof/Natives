# Blueprint: File_MilkdownEdit (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Milkdown/Crepe所见即所得MD编辑器，磁盘原文为唯一事实源，语义无损(semanticEqual)才允许富文本否则锁源码灰显切换按钮杜绝静默改写，YAML frontmatter剥离/拼回防丢，外部改盘未脏自动重载脏则保留用户改动。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/milkdownedit.rs`
...
### State Machine
```
File_MilkdownEdit:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_MilkdownEdit — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/milkdownedit.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_MilkdownEdit — 文件管理器数据模型
interface File_MilkdownEditParams {
  path: string;
  // 特征专属参数
}
interface File_MilkdownEditResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn milkdownedit(params: ...) -> Result<File_MilkdownEditResult>
```
