# Blueprint: File_ConfigSerialization (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
配置写入串行化，updateConfig用_cfgChain Promise队列串行化整个读-改-写(读也在队列内)，杜绝并发收藏+最近连发导致last-writer-wins丢更新。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/configserialization.rs`
...
### State Machine
```
File_ConfigSerialization:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ConfigSerialization — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/configserialization.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ConfigSerialization — 文件管理器数据模型
interface File_ConfigSerializationParams {
  path: string;
  // 特征专属参数
}
interface File_ConfigSerializationResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn configserialization(params: ...) -> Result<File_ConfigSerializationResult>
```
