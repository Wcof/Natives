# Blueprint: Term_ExternalLink (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
外部链接走系统浏览器不在app里开新窗口(setWindowOpenHandler)，https链接shell.openExternal打开，避免在Electron窗口内导航走。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/externallink.rs`
...
### State Machine
```
Term_ExternalLink:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_ExternalLink — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/externallink.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_ExternalLink — 终端数据模型
interface Term_ExternalLinkParams {
  id: string;
  // 特征专属参数
}
interface Term_ExternalLinkResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn externallink(params: ...) -> Result<Term_ExternalLinkResult>
```
