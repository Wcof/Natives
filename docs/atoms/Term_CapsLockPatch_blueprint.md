# Blueprint: Term_CapsLockPatch (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
xterm vendor补丁，CapsLock(20)在composition中触发提前提交导致双写，补丁加20===e.keyCode豁免，predist守卫脚本check:vendor-patch防补丁静默丢失。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/capslockpatch.rs`
...
### State Machine
```
Term_CapsLockPatch:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_CapsLockPatch — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/capslockpatch.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_CapsLockPatch — 终端数据模型
interface Term_CapsLockPatchParams {
  id: string;
  // 特征专属参数
}
interface Term_CapsLockPatchResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn capslockpatch(params: ...) -> Result<Term_CapsLockPatchResult>
```
