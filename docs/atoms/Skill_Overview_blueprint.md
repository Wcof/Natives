# Blueprint: Skill_Overview (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Skills面板总览，总skill数+唯一skill数+活跃skill数(hits>0)+总触发次数+吃灰skill数(hits=0)+问题数，30秒缓存自动刷新。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/overview.rs`
...
### State Machine
```
Skill_Overview:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Skill_Overview — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/overview.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_Overview — 技能系统数据模型
interface Skill_OverviewResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn overview(params: ...) -> Result<Skill_OverviewResult>
```
