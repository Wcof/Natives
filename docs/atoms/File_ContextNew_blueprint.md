# Blueprint: File_ContextNew (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
右键菜单新建文件/文件夹，双击空白处也可触发，validName校验(小于等于255字符+拒绝斜杠空字节+拒绝点和..)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/contextnew.rs`
...
### State Machine
```
File_ContextNew:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ContextNew — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/contextnew.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ContextNew — 文件管理器数据模型
interface File_ContextNewParams {
  path: string;
  // 特征专属参数
}
interface File_ContextNewResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn contextnew(params: ...) -> Result<File_ContextNewResult>
```
