# Blueprint: File_AtomicWrite (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
原子写与并发覆盖保护：writeTextFile用temp+fsync+rename防截断(写到一半崩溃不留残骸)；updateConfig用_cfgChain Promise队列串行化整个读-改-写杜绝last-writer-wins；写盘失败冒泡。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/atomicwrite.rs`
...
### State Machine
```
File_AtomicWrite:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_AtomicWrite — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/atomicwrite.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_AtomicWrite — 文件管理器数据模型
interface File_AtomicWriteParams {
  path: string;
  // 特征专属参数
}
interface File_AtomicWriteResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn atomicwrite(params: ...) -> Result<File_AtomicWriteResult>
```
