# Blueprint: Term_ExitCleanup (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
退出兜底清理，window-all-closed时kill所有PTY+关闭所有recorder流刷盘+恢复系统休眠(trySetDisableSleep false)；will-quit钩子确保无论怎么退(崩溃前正常退出)都恢复休眠绝不留禁休眠的烂摊子。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/exitcleanup.rs`
...
### State Machine
```
Term_ExitCleanup:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_ExitCleanup — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/exitcleanup.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_ExitCleanup — 终端数据模型
interface Term_ExitCleanupParams {
  id: string;
  // 特征专属参数
}
interface Term_ExitCleanupResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn exitcleanup(params: ...) -> Result<Term_ExitCleanupResult>
```
