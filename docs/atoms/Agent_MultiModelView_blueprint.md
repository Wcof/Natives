# Blueprint: Agent_MultiModelView (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
多模型用量同屏对比，Claude Code和Codex用量数据并列展示，各带独立进度条和统计，未来可扩展Kimi/Hermes等模型。

### Reference Code Analysis
- **Target File:** `src-tauri/src/agents_usage/multimodelview.rs`
...
### State Machine
```
Agent_MultiModelView:  idle → fetching → (success | stale | error) → idle
```
- `idle`: 初始/空闲
...
### Algorithm
```
// Agent_MultiModelView — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/agents_usage/multimodelview.rs` |
| 前端组件 | `src/components/agents-usage/` |

### Data Model
```typescript
// Agent_MultiModelView — 用量统计数据模型
interface Agent_MultiModelViewResult {
  timestamp: number;
  success: boolean;
  error?: string;
}
...
```rust
#[tauri::command]
async fn multimodelview(params: ...) -> Result<Agent_MultiModelViewResult>
```
