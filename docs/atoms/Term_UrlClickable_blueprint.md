# Blueprint: Term_UrlClickable (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端里http(s)链接可直接点击，在系统浏览器打开。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/urlclickable.rs`
...
### State Machine
```
Term_UrlClickable:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_UrlClickable — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/urlclickable.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_UrlClickable — 终端数据模型
interface Term_UrlClickableParams {
  id: string;
  // 特征专属参数
}
interface Term_UrlClickableResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn urlclickable(params: ...) -> Result<Term_UrlClickableResult>
```
