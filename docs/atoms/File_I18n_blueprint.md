# Blueprint: File_I18n (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
国际化实现，集中式翻译层326词条+73插值规则(i18n-dict.js)，默认跟系统语言侧栏可切换，界面/菜单/系统对话框全覆盖，文件名/预览/编辑器/终端用户内容不受影响。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/i18n.rs`
...
### State Machine
```
File_I18n:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_I18n — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/i18n.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_I18n — 文件管理器数据模型
interface File_I18nParams {
  path: string;
  // 特征专属参数
}
interface File_I18nResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn i18n(params: ...) -> Result<File_I18nResult>
```
