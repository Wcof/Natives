# Blueprint: Term_WechatStayAwake (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
微信离开不待机开关(wechat:setStayAwake)，独立于合盖继续运行的lidIntent，只要微信ClawBot连着就禁休眠(合盖/息屏也能远程操控)，断开微信自动恢复，首次需管理员密码装免密规则。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/wechatstayawake.rs`
...
### State Machine
```
Term_WechatStayAwake:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_WechatStayAwake — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/wechatstayawake.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_WechatStayAwake — 终端数据模型
interface Term_WechatStayAwakeParams {
  id: string;
  // 特征专属参数
}
interface Term_WechatStayAwakeResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn wechatstayawake(params: ...) -> Result<Term_WechatStayAwakeResult>
```
