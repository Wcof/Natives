# Blueprint: File_ProxyBypass (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
本地代理旁路，FanBox访问自己localhost后端时显式给loopback加代理旁路(NO_PROXY)，避免clash强制系统代理/企业PAC把本地请求拦成502导致白屏。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/proxybypass.rs`
...
### State Machine
```
File_ProxyBypass:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ProxyBypass — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/proxybypass.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ProxyBypass — 文件管理器数据模型
interface File_ProxyBypassParams {
  path: string;
  // 特征专属参数
}
interface File_ProxyBypassResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn proxybypass(params: ...) -> Result<File_ProxyBypassResult>
```
