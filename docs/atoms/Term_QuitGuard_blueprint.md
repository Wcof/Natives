# Blueprint: Term_QuitGuard (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Cmd+Q退出守卫，还有终端在跑时(agent任务)退出前弹确认对话框(还有N个终端会话在运行/退出会终止正在运行的agent任务/确定退出？)，手滑防全灭；quitConfirmed标志防二次弹窗。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/quitguard.rs`
...
### State Machine
```
Term_QuitGuard:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_QuitGuard — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/quitguard.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_QuitGuard — 终端数据模型
interface Term_QuitGuardParams {
  id: string;
  // 特征专属参数
}
interface Term_QuitGuardResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn quitguard(params: ...) -> Result<Term_QuitGuardResult>
```
