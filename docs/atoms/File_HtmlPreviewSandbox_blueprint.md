# Blueprint: File_HtmlPreviewSandbox (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
HTML预览的安全隔离架构：独立端口(主端口+1)只出文件不挂/api、sandbox iframe+allow-scripts、可读范围收紧主目录挡点目录、路径镜像端点/fs/解决相对引用裂图。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/htmlpreviewsandbox.rs`
...
### State Machine
```
File_HtmlPreviewSandbox:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_HtmlPreviewSandbox — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/htmlpreviewsandbox.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_HtmlPreviewSandbox — 文件管理器数据模型
interface File_HtmlPreviewSandboxParams {
  path: string;
  // 特征专属参数
}
interface File_HtmlPreviewSandboxResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn htmlpreviewsandbox(params: ...) -> Result<File_HtmlPreviewSandboxResult>
```
