# Blueprint: File_FollowLiveRender (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
跟随时的三种实时渲染：代码逐行高亮本次写入行并平滑滚过(hljs按行切开渲染)；HTML双缓冲(隐藏iframe加载完才换上零白闪)；MD边写边渲染尾部追加贴底滚动。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/followliverender.rs`
...
### State Machine
```
File_FollowLiveRender:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_FollowLiveRender — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/followliverender.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_FollowLiveRender — 文件管理器数据模型
interface File_FollowLiveRenderParams {
  path: string;
  // 特征专属参数
}
interface File_FollowLiveRenderResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn followliverender(params: ...) -> Result<File_FollowLiveRenderResult>
```
