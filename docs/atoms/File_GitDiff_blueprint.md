# Blueprint: File_GitDiff (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Git状态与diff只读视图，execGit调git命令，gitStatus返回变更文件列表，gitFileDiff返回单文件diff，showDiff渲染高亮。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/gitdiff.rs`
...
### State Machine
```
File_GitDiff:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/gitdiff.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_GitDiff — 文件管理器数据模型
interface File_GitDiffParams {
  path: string;
  // 特征专属参数
}
interface File_GitDiffResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn gitdiff(params: ...) -> Result<File_GitDiffResult>
```
