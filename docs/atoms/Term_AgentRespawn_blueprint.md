# Blueprint: Term_AgentRespawn (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端respawn机制，关闭标签后重新打开复用会话目录+reset，不丢上下文。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/agentrespawn.rs`
...
### State Machine
```
Term_AgentRespawn:  disconnected → connecting → (connected | failed | timeout) → disconnected
```
- `disconnected`: 未连接
...
### Algorithm
```
// Term_AgentRespawn — 异步连接（PTY spawn / 微信扫码 / OAuth）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/agentrespawn.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_AgentRespawn — 终端数据模型
interface Term_AgentRespawnParams {
  id: string;
  // 特征专属参数
}
interface Term_AgentRespawnResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn agentrespawn(params: ...) -> Result<Term_AgentRespawnResult>
```
