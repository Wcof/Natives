# Blueprint: File_DblClickFullscreen (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
双击文本/代码文件直接全屏预览(单击仍是分栏轻预览)，双击md/html路径从终端点开也直接全屏，最贴合点一下就想看清这文件长啥样的意图。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/dblclickfullscreen.rs`
...
### State Machine
```
File_DblClickFullscreen:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_DblClickFullscreen — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/dblclickfullscreen.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_DblClickFullscreen — 文件管理器数据模型
interface File_DblClickFullscreenParams {
  path: string;
  // 特征专属参数
}
interface File_DblClickFullscreenResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn dblclickfullscreen(params: ...) -> Result<File_DblClickFullscreenResult>
```
