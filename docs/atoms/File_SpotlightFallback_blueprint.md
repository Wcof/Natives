# Blueprint: File_SpotlightFallback (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Spotlight到grep双引擎降级链路，mdfind不可用或无命中时自动回退grepFiles并标注engine:grep，索引异常时给用户提示引导修复。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/spotlightfallback.rs`
...
### State Machine
```
File_SpotlightFallback:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_SpotlightFallback — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/spotlightfallback.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SpotlightFallback — 文件管理器数据模型
interface File_SpotlightFallbackParams {
  path: string;
  // 特征专属参数
}
interface File_SpotlightFallbackResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn spotlightfallback(params: ...) -> Result<File_SpotlightFallbackResult>
```
