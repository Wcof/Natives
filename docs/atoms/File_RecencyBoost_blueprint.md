# Blueprint: File_RecencyBoost (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
搜索中的近期修改加权策略，searchFiles里recencyBonus按mtime计算，grepFiles按mtime倒序读让最近写过那句话的文件优先命中，我刚做的东西浮出。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/recencyboost.rs`
...
### State Machine
```
File_RecencyBoost:  idle → typing → (debounce) → searching → (results | no_results) → idle
```
- `idle`: 搜索栏空闲
- `typing`: 用户输入
- `debounce`: 节流等待（防高频触发）
...
### Algorithm
```
// File_RecencyBoost — 异步搜索（fuzzyScore / grep / Spotlight）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/recencyboost.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_RecencyBoost — 文件管理器数据模型
interface File_RecencyBoostParams {
  path: string;
  // 特征专属参数
}
interface File_RecencyBoostResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn recencyboost(params: ...) -> Result<File_RecencyBoostResult>
```
