# Blueprint: Skill_TerminalInvoke (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
Skills终端调用，skill行拖进内嵌终端(或点详情按钮invokeSkillInTerm)，按会话里跑的agent自动注入/skill-name(Claude Code)或-name(Codex)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/skills/terminalinvoke.rs`
...
### State Machine
```
Skill_TerminalInvoke:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Skill_TerminalInvoke — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/skills/terminalinvoke.rs` |
| 前端组件 | `src/components/skills/` |

### Data Model
```typescript
// Skill_TerminalInvoke — 技能系统数据模型
interface Skill_TerminalInvokeResult {
  name: string;
  path: string;
  success: boolean;
  error?: string;
...
```rust
#[tauri::command]
async fn terminalinvoke(params: ...) -> Result<Skill_TerminalInvokeResult>
```
