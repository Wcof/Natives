# Blueprint: File_ArchivePreview (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
压缩包预览(zip/jar/tar/tgz/gz)，直接读zip中央目录按bit11判断UTF-8否则GBK解码中文文件名，不依赖系统unzip避免转码丢失。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/archivepreview.rs`
...
### State Machine
```
File_ArchivePreview:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_ArchivePreview — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/archivepreview.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ArchivePreview — 文件管理器数据模型
interface File_ArchivePreviewParams {
  path: string;
  // 特征专属参数
}
interface File_ArchivePreviewResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn archivepreview(params: ...) -> Result<File_ArchivePreviewResult>
```
