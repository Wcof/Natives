# Blueprint: File_PreviewFullscreen (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
预览全屏放大，铺满整个窗口盖住文件区/终端/侧边栏，全屏时藏macOS红黄绿系统按钮+关顶栏拖拽区防吞点击，Esc退出，图片全屏居中约束max-height避高图裁顶。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/previewfullscreen.rs`
...
### State Machine
```
File_PreviewFullscreen:  init → loading → (rendering | previewing) → (success | error)
```
- `init`: 初始化
...
### Algorithm
```
// File_PreviewFullscreen — 预览/渲染（流式加载 + 降级）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/previewfullscreen.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_PreviewFullscreen — 文件管理器数据模型
interface File_PreviewFullscreenParams {
  path: string;
  // 特征专属参数
}
interface File_PreviewFullscreenResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn previewfullscreen(params: ...) -> Result<File_PreviewFullscreenResult>
```
