# Blueprint: File_ProjectDetect (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
进入目录时自动识别项目类型(package.json->NODE/index.html->WEB/pyproject->PYTHON/Cargo.toml->RUST/go.mod->GO/.git->GIT)，面包屑旁+子目录卡片显示徽章，大目录(大于80子目录)跳过浅探避免拖慢。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/projectdetect.rs`
...
### State Machine
```
File_ProjectDetect:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ProjectDetect — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/projectdetect.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ProjectDetect — 文件管理器数据模型
interface File_ProjectDetectParams {
  path: string;
  // 特征专属参数
}
interface File_ProjectDetectResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn projectdetect(params: ...) -> Result<File_ProjectDetectResult>
```
