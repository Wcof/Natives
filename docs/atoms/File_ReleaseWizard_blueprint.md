# Blueprint: File_ReleaseWizard (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
发版向导，node项目状态条出现发版——版本号(预填patch+1)、发布说明(预填CHANGELOG Unreleased段)、打包/推送/GitHub Release可勾选，确认后整条命令序列在内嵌终端开跑每步可见可拦。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/releasewizard.rs`
...
### State Machine
```
File_ReleaseWizard:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_ReleaseWizard — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/releasewizard.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_ReleaseWizard — 文件管理器数据模型
interface File_ReleaseWizardParams {
  path: string;
  // 特征专属参数
}
interface File_ReleaseWizardResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn releasewizard(params: ...) -> Result<File_ReleaseWizardResult>
```
