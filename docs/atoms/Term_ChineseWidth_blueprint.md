# Blueprint: Term_ChineseWidth (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
终端中文宽度修复，Unicode11 addon处理CJK双宽字符，全角标点进路径边界字符集。

### Reference Code Analysis
- **Target File:** `src-tauri/src/terminal/chinesewidth.rs`
...
### State Machine
```
Term_ChineseWidth:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// Term_ChineseWidth — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/terminal/chinesewidth.rs` |
| 前端组件 | `src/components/terminal/` |

### Data Model
```typescript
// Term_ChineseWidth — 终端数据模型
interface Term_ChineseWidthParams {
  id: string;
  // 特征专属参数
}
interface Term_ChineseWidthResult {
  success: boolean;
  id: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn chinesewidth(params: ...) -> Result<Term_ChineseWidthResult>
```
