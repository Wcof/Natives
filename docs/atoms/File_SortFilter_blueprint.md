# Blueprint: File_SortFilter (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
排序(名称/修改时间/大小，文件夹恒在前，localeCompare zh numeric)+当前目录即时筛选(/聚焦)+键盘导航(上下左右选择/回车预览/Cmd回车编辑器打开/Backspace上级)。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/sortfilter.rs`
...
### State Machine
```
File_SortFilter:  clean → dirty → saving → (saved | conflict | error) → clean
```
- `clean`: 初始/已保存状态
- `dirty`: 内容已修改
- `saving`: 正在落盘（原子写）
...
### Algorithm
```
// File_SortFilter — 原子写 + conflict guard + undo stack
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/sortfilter.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_SortFilter — 文件管理器数据模型
interface File_SortFilterParams {
  path: string;
  // 特征专属参数
}
interface File_SortFilterResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn sortfilter(params: ...) -> Result<File_SortFilterResult>
```
