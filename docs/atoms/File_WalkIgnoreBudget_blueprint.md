# Blueprint: File_WalkIgnoreBudget (Repaired — Feature-Specific)

## 1. Logic Deconstruction & Reference Analysis

### Experience
递归遍历walk()的工程化控制：忽略表(node_modules/.git/dist等)、结果上限limit、时间预算deadline、截断标志truncated前端展示结果可能不完整警告。

### Reference Code Analysis
- **Target File:** `src-tauri/src/file_manager/walkignorebudget.rs`
...
### State Machine
```
File_WalkIgnoreBudget:  idle → processing → (success | error) → idle
```
- `idle`: 初始/空闲状态
- `processing`: 执行业务逻辑
- `success`: 操作成功
...
### Algorithm
```
// File_WalkIgnoreBudget — 增量数据获取（带 offset 缓存 + stale 检测）
...
| 层 | 路径 |
|---|------|
| Rust 后端 | `src-tauri/src/file_manager/walkignorebudget.rs` |
| 前端组件 | `src/components/files/` |

### Data Model
```typescript
// File_WalkIgnoreBudget — 文件管理器数据模型
interface File_WalkIgnoreBudgetParams {
  path: string;
  // 特征专属参数
}
interface File_WalkIgnoreBudgetResult {
  success: boolean;
  path: string;
  error?: string;
}
...
```rust
#[tauri::command]
async fn walkignorebudget(params: ...) -> Result<File_WalkIgnoreBudgetResult>
```
