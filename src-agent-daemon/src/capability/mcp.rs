...
/// Resolve the update sentinel for env/headers maps: a JSON `null` value means
/// "keep the stored value for this key" (the daemon never echoes plaintext, so
/// the UI cannot send unchanged rows back). Absent keys still mean deletion;
/// `null` for a key that has no stored value is rejected (fail-closed).
fn merge_kept_kv(field: &str, patch: &Value, stored: Option<&Value>) -> Result<Value, String> {
    let Some(patch_map) = patch.as_object() else {
        return Ok(patch.clone());
    };
    let empty = serde_json::Map::new();
    let stored_map = stored.and_then(Value::as_object).unwrap_or(&empty);
    let mut out = serde_json::Map::new();
    for (k, v) in patch_map {
        if v.is_null() {
            let Some(kept) = stored_map.get(k) else {
                return Err(format!("cannot keep unknown {field} key: {k}"));
            };
            out.insert(k.clone(), kept.clone());
        } else {
...
pub fn update(params_value: &Value) -> Result<Value, String> {
    ...
    for key in [
        ...
        if let Some(v) = params_value.get(key) {
            let v = if matches!(key, "env" | "headers") {
                merge_kept_kv(key, v, merged.get(key))?
            } else {
                v.clone()
            };
