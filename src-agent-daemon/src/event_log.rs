...
impl EventLog {
    ...
    fn append_event_v2_with_conn(
        ...
        // an external CLI (native-first design). W2: projection lives in the
        // explicit projector, not in the EventLog append path.
        if let RunEventKind::UsageUpdated { .. } = &event.payload {
            crate::conversation_projector::project_usage_rollup(conn, event)?;
        }
        Ok(global)
    }
