#!/usr/bin/env bash
# Linux: start natives-agent-daemon sidecar with secure bootstrap (not logged).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
RUNTIME_DIR="${NATIVES_RUNTIME_DIR:-${XDG_RUNTIME_DIR:-$HOME/.natives/runtime}/natives}"
# Prefer XDG_RUNTIME_DIR when set
if [[ -n "${XDG_RUNTIME_DIR:-}" && -z "${NATIVES_RUNTIME_DIR:-}" ]]; then
  RUNTIME_DIR="$XDG_RUNTIME_DIR/natives"
fi
mkdir -p "$RUNTIME_DIR"
chmod 700 "$RUNTIME_DIR" 2>/dev/null || true

SOCKET="${NATIVES_DAEMON_SOCKET:-$RUNTIME_DIR/natives-agent.sock}"
# Guard sun_path length (also on Linux abstract/path sockets).
if [[ ${#SOCKET} -gt 100 ]]; then
  SOCKET="/tmp/nuds-$(printf '%s' "$RUNTIME_DIR" | sha256sum 2>/dev/null | cut -c1-12 || echo $$).sock"
  echo "NOTE: socket path shortened to $SOCKET (UDS path limit)"
fi
