...
try {
  ...
  assert.ok(!existsSync(demoManifest), 'resource apps must not register a child Native Host');
  ...
  // Uninstall while the App tab is open verifies page coordination and shared Host EOF cleanup.
  ...
  assert.ok(!existsSync(personal));
  console.log(JSON.stringify({ chromeNativeMessaging: true, version: entry.version, install: 'passed',
    noChildHostRegistration: 'passed', resourceRead: 'passed', liveAppUninstall: 'passed', elapsedMs: Math.round(performance.now() - started), retainedDataPurge: 'passed' }));
} catch (error) {
