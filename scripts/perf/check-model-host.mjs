...
if (process.platform !== 'win32') {
  const cpuStart = processCPUTime(child.pid);
  const idleStartedAt = performance.now();
  await new Promise((resolvePromise) => setTimeout(resolvePromise, 1000));
  const cpuEnd = processCPUTime(child.pid);
  ...
  rssKB = Number.parseInt(result.stdout.trim(), 10) || null;
}
...
async function checkResidentLifecycle(directory) {
  ...
  const enabled = await withTimeout(primaryClient.call({ id: 'resident-on', method: 'model_gateway_set_resident', params: { expectedRevision: 1, resident: true } }), 3000, 'resident enable timed out', () => primary.kill('SIGKILL'));
  primary.stdin.end();
  await new Promise((resolvePromise) => setTimeout(resolvePromise, 250));
  const stayedRunning = primary.exitCode === null;
  const relay = spawn(binary, [], { env: environment, stdio: ['pipe', 'pipe', 'ignore'] });
  const relayClient = framedClient(relay);
  const snapshot = await withTimeout(relayClient.call({ id: 'resident-snapshot', method: 'model_snapshot', params: {} }), 3000, 'resident relay timed out', () => relay.kill('SIGKILL'));
  const revision = snapshot?.result?.revision;
  const disabled = await withTimeout(relayClient.call({ id: 'resident-off', method: 'model_gateway_set_resident', params: { expectedRevision: revision, resident: false } }), 3000, 'resident disable timed out', () => relay.kill('SIGKILL'));
  relay.stdin.end();
  const [relayExit, primaryExit] = await Promise.all([
    withTimeout(waitForExit(relay), 2000, 'resident relay did not exit', () => relay.kill('SIGKILL')),
    withTimeout(waitForExit(primary), 2000, 'resident primary did not exit', () => primary.kill('SIGKILL')),
  ]);
  return { ok: enabled?.ok === true && stayedRunning && snapshot?.result?.gateway?.resident === true && disabled?.result?.gateway?.resident === false && relayExit.code === 0 && primaryExit.code === 0, stayedRunning, reusedPrimary: snapshot?.result?.gateway?.resident === true, relayExit, primaryExit };
}
...
function waitForExit(childProcess) {
  if (childProcess.exitCode !== null) return Promise.resolve({ code: childProcess.exitCode, signal: childProcess.signalCode });
