...
const STAGING_DIR = join(DIST_DIR, 'Natives-Windows-x64-Local');
const ZIP_PATH = join(DIST_DIR, 'Natives-Windows-x64-Local.zip');
...
// ADR-0031：唯一产品级 App Runtime 可执行文件（Windows 等价交付）。
const APP_RUNTIME_EXE = join(ROOT, 'target', WINDOWS_TARGET, 'release', 'natives-app-runtime.exe');

console.log('=======================================================');
console.log('  Natives Windows x64 本地候选包打包程序 (macOS 本地)');
console.log('=======================================================\n');
...
async function main() {
  ...
  console.log('  -> 编译 natives-app-runtime.exe (Rust, ADR-0031 统一 Runtime)...');
  run('cargo', ['build', '-p', 'app-runtime', '--release', '--target', WINDOWS_TARGET], {}, 'natives-app-runtime 编译失败');
  ...
  verifyPE32Plus(APP_RUNTIME_EXE, 'natives-app-runtime.exe');
  console.log('  √ 验证成功：三个二进制文件均为标准 Windows PE32+ (x86-64) 可执行文件');
  ...
  const excludedNames = new Set([
    ...
    'install-native-host.mjs', 'launch-natives.mjs', 'native-host-manifest.json',
    'folder-source.svg', 'test-dom-mock.js',
  ]);

  buildExtension(join(STAGING_DIR, 'Extension'), ROOT, 'local');
  ...
  // 统一 App Runtime（计划 §36.2：Windows 包必须包含 natives-app-runtime.exe）
  await cp(APP_RUNTIME_EXE, join(STAGING_DIR, 'Host', 'natives-app-runtime.exe'));
...
`;
  ...
  // install-host.ps1
  await writeFile(join(STAGING_DIR, 'install-host.ps1'), `# [Natives] Windows 本地候选包安装与注册脚本
...
# 2. 复制文件到 %LOCALAPPDATA%\\Natives-Local
$targetDir = Join-Path $env:LOCALAPPDATA "Natives-Local"
...
$appRuntimeExe = (Join-Path $targetDir "Host\\natives-app-runtime.exe").Replace('\\', '\\\\')

$fileManifestContent = @'
{
  "name": "com.natives.local.file_manager",
...
{
  "name": "com.natives.local.model_host",
...
# ADR-0031：唯一产品级 App Runtime Native Messaging Host
# （计划 §36.3：Registry 默认值指向 Manifest JSON，Manifest path 指向 exe）
$appRuntimeManifestContent = @'
{
  "name": "com.natives.local.app_runtime",
  "description": "Natives unified built-in app runtime",
  "path": "@APP_RUNTIME_EXE@",
  "type": "stdio",
  "allowed_origins": [
    "chrome-extension://@EXT_ID@/"
  ]
}
'@.Replace('@APP_RUNTIME_EXE@', $appRuntimeExe).Replace('@EXT_ID@', $extensionId)

$fileManifestPath = Join-Path $targetDir "manifests\\com.natives.local.file_manager.json"
$modelManifestPath = Join-Path $targetDir "manifests\\com.natives.local.model_host.json"
$appRuntimeManifestPath = Join-Path $targetDir "manifests\\com.natives.local.app_runtime.json"
...
[System.IO.File]::WriteAllText($appRuntimeManifestPath, $appRuntimeManifestContent, [System.Text.Encoding]::UTF8)

# 写入当前用户注册表
$regBasePath = "HKCU:\\Software\\Google\\Chrome\\NativeMessagingHosts"
New-Item -Path "$regBasePath\\com.natives.local.file_manager" -Force | Out-Null
Set-ItemProperty -Path "$regBasePath\\com.natives.local.file_manager" -Name "(Default)" -Value $fileManifestPath -Force

New-Item -Path "$regBasePath\\com.natives.local.model_host" -Force | Out-Null
Set-ItemProperty -Path "$regBasePath\\com.natives.local.model_host" -Name "(Default)" -Value $modelManifestPath -Force

New-Item -Path "$regBasePath\\com.natives.local.app_runtime" -Force | Out-Null
Set-ItemProperty -Path "$regBasePath\\com.natives.local.app_runtime" -Name "(Default)" -Value $appRuntimeManifestPath -Force
...
`);
  ...
  // uninstall-host.ps1
  await writeFile(join(STAGING_DIR, 'uninstall-host.ps1'), `# [Natives] Windows 本地候选包卸载脚本
$OutputEncoding = [System.Text.Encoding]::UTF8
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
...
$regBasePath = "HKCU:\\Software\\Google\\Chrome\\NativeMessagingHosts"
Remove-Item -Path "$regBasePath\\com.natives.local.file_manager" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$regBasePath\\com.natives.local.model_host" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item -Path "$regBasePath\\com.natives.local.app_runtime" -Recurse -Force -ErrorAction SilentlyContinue
...
$appRuntimeExe = Join-Path $targetDir "Host\\natives-app-runtime.exe"
if (Test-Path $appRuntimeExe) {
    Write-Host "  [√] natives-app-runtime.exe 存在 (ADR-0031 统一 Runtime)" -ForegroundColor Green
} else {
    Write-Host "  [×] natives-app-runtime.exe 未找到" -ForegroundColor Red
}

Write-Host "\`n3. 注册表与 Manifest 关联检查:"
$fileReg = (Get-ItemProperty -Path "$regBasePath\\com.natives.local.file_manager" -ErrorAction SilentlyContinue)."(Default)"
$modelReg = (Get-ItemProperty -Path "$regBasePath\\com.natives.local.model_host" -ErrorAction SilentlyContinue)."(Default)"
$appRuntimeReg = (Get-ItemProperty -Path "$regBasePath\\com.natives.local.app_runtime" -ErrorAction SilentlyContinue)."(Default)"

if ($fileReg -and (Test-Path $fileReg)) {
    Write-Host "  [√] com.natives.local.file_manager 注册有效: $fileReg" -ForegroundColor Green
} else {
    Write-Host "  [×] com.natives.local.file_manager 注册缺失或文件不存在" -ForegroundColor Red
}

if ($modelReg -and (Test-Path $modelReg)) {
    Write-Host "  [√] com.natives.local.model_host 注册有效: $modelReg" -ForegroundColor Green
} else {
    Write-Host "  [×] com.natives.local.model_host 注册缺失或文件不存在" -ForegroundColor Red
}

if ($appRuntimeReg -and (Test-Path $appRuntimeReg)) {
    Write-Host "  [√] com.natives.local.app_runtime 注册有效: $appRuntimeReg" -ForegroundColor Green
} else {
    Write-Host "  [×] com.natives.local.app_runtime 注册缺失或文件不存在" -ForegroundColor Red
}
...
3. 安装程序会自动：
   - 将运行文件复制到 %LOCALAPPDATA%\\Natives-Local
...
`;
  ...
  run('zip', ['-r', '-X', '-q', ZIP_PATH, 'Natives-Windows-x64-Local'], { cwd: DIST_DIR }, '创建 ZIP 压缩包失败');

  const zipStat = await stat(ZIP_PATH);
  const zipHash = createHash('sha256').update(await readFile(ZIP_PATH)).digest('hex');

  console.log('\n=======================================================');
  console.log('  🎉 Windows x64 本地候选包构建完成！');
  console.log('=======================================================');
