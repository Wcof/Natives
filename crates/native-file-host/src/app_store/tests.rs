...
fn demo_install_query_enable_sidebar_uninstall_cycle() {
    ...
    assert_eq!(store.app(&app.app_id).unwrap_err().code(), "APP_NOT_FOUND");
...
fn begin_rejects_unknown_kind_and_bad_app_id() {
    ...
    assert_eq!(
        store.install_begin(&bad_kind).unwrap_err().code(),
        "APP_INVALID_STATE"
    );
...
fn double_install_is_a_conflict() {
    ...
    let error = store.install_begin(&demo_request()).unwrap_err();
    assert_eq!(error.code(), "APP_CONFLICT");

    // committing twice is also a conflict
    let error = store.install_commit(&tx.install_id).unwrap_err();
    assert_eq!(error.code(), "APP_CONFLICT");
}
...
fn commit_and_abort_require_a_known_transaction() {
    ...
    assert_eq!(
        store.install_commit("missing").unwrap_err().code(),
        "APP_NOT_FOUND"
    );
    assert_eq!(
        store
            .install_abort("missing", "APP_IO", "no such install")
            .unwrap_err()
            .code(),
        "APP_NOT_FOUND"
    );
    assert_eq!(
        store.uninstall("missing").unwrap_err().code(),
        "APP_NOT_FOUND"
    );
}
