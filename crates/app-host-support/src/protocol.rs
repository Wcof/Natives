//! App v1 / Core Apps v4 / Catalog v3 协议常量与 wire 类型（契约 v1 §4.0/§5.2）。
//!
...
/// 独立应用运行协议版本（App protocol v1）。
pub const APP_PROTOCOL_VERSION: u32 = 1;
/// 安装管理协议版本（Core Apps protocol v4）。
pub const CORE_APPS_PROTOCOL_V4: u32 = 4;
/// 目录版本（Catalog v3，与 v2 显式隔离）。
pub const CATALOG_VERSION_V3: u32 = 3;

/// Native/App 单帧上限：严格小于 Chrome 1 MiB 出站上限。
pub const MAX_FRAME_BYTES: usize = 512 * 1024;
/// 安装分块大小（原始 gzip artifact 字节）。
...
/// Catalog v3 条目的交付声明（托管应用必需字段；校验在 Core 侧执行）。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CatalogV3Package {
    pub platform: String,
    pub arch: String,
    pub kind: String,
    pub url: String,
...
mod tests {
    ...
    fn request_round_trip_and_unknown_shape() {
        ...
        assert_eq!(APP_PROTOCOL_VERSION, 1);
        assert_eq!(CORE_APPS_PROTOCOL_V4, 4);
        assert_eq!(CATALOG_VERSION_V3, 3);
        assert_eq!(MAX_FRAME_BYTES, 512 * 1024);
        assert_eq!(INSTALL_CHUNK_SIZE, 262_144);
    }
