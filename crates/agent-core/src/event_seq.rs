...
pub(crate) mod test_support {
    ...
    /// In-memory `EventPersistence` for tests: no disk, no `~/.natives/events`
    /// reads or writes. Tests that assert on event streams must use
    /// [`memory_sequencer`] so they never depend on stale home event history.
    #[derive(Default)]
    pub struct MemoryPersistence {
        by_run: Mutex<HashMap<String, Vec<RunEventV2>>>,
    }
    ...
    impl EventPersistence for MemoryPersistence {
        fn append(&self, event: &RunEventV2) -> Result<(), String> {
            self.by_run
                .lock()
                .expect("memory persistence lock")
                .entry(event.run_id.clone())
                .or_default()
                .push(event.clone());
            Ok(())
    ...
    /// A sequencer backed by [`MemoryPersistence`] — deterministic, isolated,
    /// and independent of `NATIVES_EVENT_LOG_*` / `~/.natives`.
    pub fn memory_sequencer() -> EventSequencer {
        EventSequencer::with_persistence(Arc::new(MemoryPersistence::default()))
    }
}
